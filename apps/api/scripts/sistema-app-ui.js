...
(function () {
  ...
  // §11 da refatoração: chave lógica LOJA + ID_RESGATE + ID_PEDIDO. Agrupar por resgate NUNCA
  // deduplica por data — vários resgates podem cair no mesmo dia, cada um contado uma vez.
  ...
  // §7-8 da refatoração: agregação por DATA DO RESGATE — usada só para agrupar visualmente o
  // gráfico/resumo "dia a dia"; nunca é a chave de deduplicação (essa é loja+resgate+pedido).
  function aceleraByDia(recs) {
    var map = {};
    recs.forEach(function (r) {
      var g = map[r.data] = map[r.data] || { data: r.data, pedidosSet: {}, resgatesSet: {}, antec: 0, taxa: 0, receb: 0, reemb: 0, pend: 0 };
      g.pedidosSet[r.pedido] = 1; g.resgatesSet[r.resgate] = 1;
      g.antec += r.antecipado; g.taxa += r.taxa; g.receb += r.recebido; g.reemb += r.reembolsado; g.pend += r.pendente;
    });
    return Object.keys(map).sort().map(function (k) {
      var g = map[k]; var pedidos = Object.keys(g.pedidosSet).length, resgates = Object.keys(g.resgatesSet).length;
      var liquidoCalc = g.antec - g.taxa;
      return { data: g.data, pedidos: pedidos, resgates: resgates, antec: g.antec, taxa: g.taxa, receb: g.receb, liquidoCalc: liquidoCalc, liquidoDiff: g.receb - liquidoCalc, reemb: g.reemb, pend: g.pend, aliquota: g.antec ? g.taxa / g.antec : 0 };
    });
  }
  ...
  function aceleraVisao() {
    ...
    var strip1 = kstrip([
      ...
      { l: 'Valor antecipado', v: brlC(m.volume), cls: 'blue' },
  ...
  // §8-9: expansão do dia → resgates do dia → pedidos do resgate (reaproveita openAceleraResgate).
  function openAceleraDia(dateStr) {
    var recs = acelera.filter(function (r) { return r.data === dateStr; });
    ...
    panel.innerHTML = '<div class="dh"><div><b>' + esc(dbr(dateStr)) + '</b></div><button class="x">&times;</button></div><div class="dbd">' +
  ...
  function aceleraAntecipacoes() {
    ...
    var rows = slice.map(function (g) { return '<tr class="rowlink" data-acresg="' + esc(g.resgate) + '"><td class="nowrap">' + esc(dbr(g.data)) + '</td><td class="mono">' + esc(g.resgate) + '</td><td>' + nn(g.n) + '</td><td class="nowrap">' + brlC(g.antec) + '</td><td class="nowrap">' + brlC(g.taxa) + '</td><td><span class="tag' + (g.divergente ? ' warn' : '') + '">' + g.aliqLabel + '</span></td><td class="nowrap">' + brlC(g.receb) + '</td><td class="nowrap ' + (g.reemb ? 'neg' : '') + '">' + brlC(g.reemb) + '</td><td class="nowrap">' + brlC(g.pend) + '</td><td><button class="btn-sm" data-acresg="' + esc(g.resgate) + '">Abrir</button></td></tr>'; }).join('');
    ...
    return head +
      ...
      '<div class="panel"><div class="table-wrap"><table class="report"><thead><tr><th>Data</th><th>ID resgate</th><th>Pedidos</th><th>Antecipado</th><th>Taxa</th><th>Alíquota</th><th>Líquido</th><th>Reembolsado</th><th>Pendente</th><th></th></tr></thead><tbody>' + (rows || '<tr><td colspan="10" class="empty">Nenhum resgate neste filtro.</td></tr>') + '</tbody></table></div></div>' +
  ...
  function openAceleraPedido(pid) {
    ...
    if (r) {
      ...
      antBlock = '<div class="panel"><div class="ph"><h3>Antecipação</h3></div><div class="pb">' + kv('Resgate', r.resgate) + kv('Data', dbr(r.data)) + kv('Valor antecipado', brlC(r.antecipado)) + kv('Taxa', brlC(r.taxa)) + kv('Alíquota', pct(r2((r.antecipado ? r.taxa / r.antecipado : 0) * 100))) + kv('Líquido informado (Shopee)', brlC(r.recebido)) + kv('Líquido calculado (antecipado − taxa)', brlC(liqCalc)) + (diverge ? kv('Diferença', brlC(liqDiff)) : '') + '</div></div>' +
        ...
        '<div class="panel"><div class="ph"><h3>Depois da antecipação</h3></div><div class="pb">' + kv('Reembolsado', brlC(r.reembolsado)) + kv('Pendente', brlC(r.pendente)) + kv('Última transação', dbr(r.ultimaTransacao)) + kv('Vencimento', dbr(r.vencimento)) + (r.reembolsado > 0 ? '<div style="margin-top:4px"><span class="tag warn">Pedido posteriormente reembolsado</span></div>' : '') + '</div></div>' +
  ...
  function aceleraDivergencias() {
    ...
    else if (sel === 'acSemExp') body = aceleraDivTable(aceleraSemExpedicao.map(function (oid) { var r = acByOrder[oid][0]; return { pedido: oid, motivo: 'Antecipado em ' + dbr(r.data) + ', sem expedição/bipe registrado' }; }));
  ...
  function aceleraAuditoria() {
    ...
    return head + finBlock + cntBlock + concBlock + recon + periodo + qual;
  }
