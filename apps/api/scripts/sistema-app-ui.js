...
(function () {
  ...
  // ---- MULTI-CNPJ / MULTI-OPERAÇÃO / MULTI-LOJA ----
  // Empresa (CNPJ) → Operação (loja/marketplace) — hierarquia aditiva, sem quebrar nenhum dado
  // existente. orders/wallet/acelera/mrRenda/occ/affConv/... ganham operationId (campo novo) e são
  // FILTRADOS em memória no boot pela operação ativa — cada motor de negócio já existente (centenas
  // de funções) continua lendo os MESMOS arrays globais sem nenhuma alteração, porque o array só
  // contém os registros da operação ativa (ou de todas, no modo consolidado). Isso evita reescrever
  // cada função para aceitar operationId — a fronteira multi-tenant vive só no boot/import/UI.
  var OP_ALL = '__ALL__'; // sentinela do modo "Todas as Operações" (agregação — nunca mistura o dado gravado)
  var companies = []; // { id, razaoSocial, nomeFantasia, cnpj, inscricaoEstadual, observacao, ativa, createdAt }
  var operations = []; // { id, companyId, nome, marketplace, storeName, sellerId, ativa, createdAt }
  var activeOperationId = null; // null só antes do boot terminar; depois, um id real ou OP_ALL
  function opActive() { return operations.find(function (o) { return o.id === activeOperationId; }) || null; }
  function opCompany(op) { return op ? companies.find(function (c) { return c.id === op.companyId; }) : null; }
  function opLabel(op) { var c = opCompany(op); return op ? ((c ? c.nomeFantasia || c.razaoSocial : '—') + ' · ' + (op.marketplace || op.nome)) : '—'; }
  // Toda importação exige uma operação ESPECÍFICA selecionada — nunca "Todas as Operações" (§AV do
  // complemento). Cada import*() já tem `.catch(function(e){ toast('Falha', e.message, true); })`
  // no chamador — rejeitar com uma mensagem clara reaproveita esse mecanismo, sem duplicar toast.
  // Nunca lança síncrono (quebraria a cadeia de Promise do import*) — cada import*() checa
  // opActiveOrNull() no início e devolve Promise.reject() explicitamente quando bloqueado.
  function opActiveOrNull() { return (activeOperationId && activeOperationId !== OP_ALL) ? activeOperationId : null; }
  var OP_BLOCK_MSG = 'Escolha uma operação específica (empresa/loja) no topo antes de importar este arquivo.';
  ...
  // Causa raiz do bug "Caixa mostra o Acelera do dia errado": datas SEM hora (ex.: dateKey "2026-08-18",
  // vindas de orders.paidAt.slice(0,10), expSessions.dataOperacional, aceleraByDia().data) eram
  // formatadas com `new Date(str)`, que o JS interpreta como UTC meia-noite — em fusos negativos
  // (America/Sao_Paulo) isso exibe o dia ANTERIOR. O dado em si (a chave usada para buscar o Acelera)
  // sempre esteve correto; só o RÓTULO ficava errado, fazendo o botão do dia 19/08 exibir "18/08" e
  // abrir com os números corretos de 19/08 — parecendo, na tela, "18/08 mostrando dado de 19/08".
  // Corrigido na raiz (mesma técnica já usada em acLocalDate, agora generalizada): datas puras
  // YYYY-MM-DD são sempre interpretadas como LOCAIS; qualquer outro formato (timestamp completo com
  // hora/offset, Date, epoch) mantém o comportamento original. Nenhum offset manual (+1/-1) foi usado.
  function dbr(d) { if (!d) return '—'; var s = typeof d === 'string' ? d : null; var bare = s && /^\d{4}-\d{2}-\d{2}$/.test(s.slice(0, 10)) && s.length <= 10; var x = bare ? new Date(s + 'T00:00:00') : new Date(d); return isNaN(x) ? '—' : x.toLocaleDateString('pt-BR'); }
  ...
  // §22-31 do prompt "Correção definitiva de SKU/Variações/Famílias/Custo": o ÍNDICE nunca decide
  // conflito — só REGISTRA todas as candidatas (ativas e inativas) que reivindicam uma chave de SKU,
  // com status do anúncio, productId e matchedBy. Quem decide é resolveSkuCostByKey(), por prioridade
  // determinística. Isso separa corretamente "existe mais de uma candidata" (fato, sempre guardado)
  // de "qual delas vale" (regra, aplicada na leitura — nunca perdida ao reindexar).
  function rebuildSkuCost() {
    ...
    function addCandidate(rawSku, matchedBy, v) {
      ...
      if (list.some(function (c) { return c.variationId === v.id && c.matchedBy === matchedBy; })) return;
      list.push({ variationId: v.id, productId: v.productId, status: prodStatusById[v.productId] || 'ACTIVE', matchedBy: matchedBy });
    }
    data.variations.forEach(function (v) { addCandidate(v.sku, 'SKU', v); addCandidate(v.referenceSku, 'REFERENCE_SKU', v); });
    ...
    Object.keys(skuAliasIndex).forEach(function (key) {
      var r = resolveSkuCostByKey(key, key);
      skuCost[key] = { linked: true, cost: r.found ? r.cost : null, familyName: r.familyName || null };
      if (/^SKU_CONFLIT/.test(r.motivo || '')) skuConflicts[key] = true; // auditável via resolveSkuCost().motivo
    });
  }
  ...
  function resolveSkuCostByKey(key, requestedSku, itemHint) {
    ...
    function finalize(cand, regra) {
      ...
      if (!v.familyId) return { found: false, requestedSku: requestedSku, matchedBy: cand.matchedBy, canonicalSku: v.sku, variationId: v.id, productId: v.productId, motivo: 'SEM_FAMILIA', regra: regra };
      var f = skuFamById[v.familyId];
      if (!f || f.currentCostAmount == null) return { found: false, requestedSku: requestedSku, matchedBy: cand.matchedBy, canonicalSku: v.sku, variationId: v.id, productId: v.productId, familyId: v.familyId, familyName: f ? f.name : null, motivo: 'FAMILIA_SEM_CUSTO', regra: regra };
    ...
    if (activeSku.length > 1) return { found: false, requestedSku: requestedSku, motivo: 'SKU_CONFLITANTE', candidatos: activeSku };
    var activeRef = tier('REFERENCE_SKU', true);
    if (activeRef.length === 1) return finalize(activeRef[0], 'ACTIVE_REFERENCE_SKU');
    if (activeRef.length > 1) return { found: false, requestedSku: requestedSku, motivo: 'SKU_CONFLITANTE', candidatos: activeRef };
    var inactiveSku = tier('SKU', false);
    if (inactiveSku.length === 1) return finalize(inactiveSku[0], 'HISTORICAL_INACTIVE_MATCH');
    if (inactiveSku.length > 1) return { found: false, requestedSku: requestedSku, motivo: 'SKU_CONFLITANTE_HISTORICAL', candidatos: inactiveSku };
    var inactiveRef = tier('REFERENCE_SKU', false);
    if (inactiveRef.length === 1) return finalize(inactiveRef[0], 'HISTORICAL_INACTIVE_MATCH');
    if (inactiveRef.length > 1) return { found: false, requestedSku: requestedSku, motivo: 'SKU_CONFLITANTE_HISTORICAL', candidatos: inactiveRef };
  ...
  var SKU_MOTIVO_LABEL = { SKU_NAO_LOCALIZADO: 'SKU não localizado em Produtos', SEM_FAMILIA: 'Família não atribuída', FAMILIA_SEM_CUSTO: 'Custo da família não informado', SKU_CONFLITANTE: 'Conflito de SKU — mais de um anúncio ATIVO usa o mesmo código', SKU_CONFLITANTE_HISTORICAL: 'Conflito de SKU — mais de um anúncio INATIVO usa o mesmo código, sem identificador para desempatar' };
  ...
  function importPosVenda(type, file) {
    ...
    return file.arrayBuffer().then(function (ab) {
      ...
      Object.keys(groups).forEach(function (key) {
        ...
        var ex = byId[uid];
        if (!ex) {
          var o = newOcc(uid, type); o.operationId = opId; o.orderId = rep.orderId; o.returnId = rep.returnId;
  ...
  // §41-49 do prompt "Correção Financeira": normalização + resolução canônica do cruzamento
  // Pedido×Income. A chave correta nunca é só orderId — é operationId (da PRÓPRIA operação do
  // pedido, não a operação ativa no seletor — importa em modo "Todas as Operações") + orderId
  // normalizado + ver==='Order'. Nunca reduz tudo a um "sem Income cruzado" genérico: distingue
  // por que o cruzamento não aconteceu, para nunca esconder um bug real atrás de uma mensagem vaga.
  function normalizeOrderId(value) {
    if (value == null) return '';
    return String(value).replace(/[​-‍﻿\xa0]/g, '').trim();
  }
  var INCOME_RESOLVE_LABEL = {
    FOUND: 'Encontrado no Income', NOT_IN_INCOME: 'Ainda não lançado no Income',
    WRONG_OPERATION: 'Existe no Income, mas em outra operação', DUPLICATE_ORDER_ROWS: 'Mais de uma linha "Order" para este pedido no Income',
    INVALID_ORDER_ID: 'ID do pedido inválido',
  };
  function resolveIncomeOrder(operationId, orderId) {
    var key = normalizeOrderId(orderId);
    if (!key) return { status: 'INVALID_ORDER_ID', row: null, requestedOrderId: orderId };
    var matches = mrRenda.filter(function (r) { return r.ver === 'Order' && normalizeOrderId(r.orderId) === key; });
    if (!matches.length) return { status: 'NOT_IN_INCOME', row: null, requestedOrderId: orderId };
    // registros sem operationId (dado legado pré-migração) são visíveis em qualquer operação —
    // mesma regra defensiva já usada no filtro de boot multi-operação (§Fase 10-12).
    var sameOp = matches.filter(function (r) { return !operationId || !r.operationId || r.operationId === operationId; });
    if (!sameOp.length) return { status: 'WRONG_OPERATION', row: null, requestedOrderId: orderId, foundInOperationId: matches[0].operationId };
    if (sameOp.length > 1) return { status: 'DUPLICATE_ORDER_ROWS', row: sameOp[0], requestedOrderId: orderId, count: sameOp.length };
    return { status: 'FOUND', row: sameOp[0], requestedOrderId: orderId };
  }
  function pedidoIncomeRow(orderId) {
    var ord = orders.find(function (x) { return x.id === orderId; });
    var opId = (ord && ord.operationId) || opActiveOrNull() || null;
    var res = resolveIncomeOrder(opId, orderId);
    return res.status === 'FOUND' ? res.row : null;
  }
  // Variante que também devolve o diagnóstico completo — usada pela Ficha do Pedido (Conciliação)
  // e pelo status financeiro; pedidoIncomeRow() continua existindo para os consumidores que só
  // precisam do valor (nunca duplica a busca — delega para a mesma resolveIncomeOrder()).
  function pedidoIncomeResolve(orderId) {
    var ord = orders.find(function (x) { return x.id === orderId; });
    var opId = (ord && ord.operationId) || opActiveOrNull() || null;
  ...
  function pedidoComposicaoFinanceira(orderId) {
    ...
    // §59 do prompt "Correção Financeira": status financeiro único, nunca "aproximado" tratado como
    // definitivo — CONFIRMADO (Income encontrado e a matemática fecha), DIVERGÊNCIA (Income encontrado
    // mas a Quantia Total Lançada não bate com a composição — pendência real) ou PROVISÓRIO (Income
  ...
  function auditOrderProductCost(orderId) {
    ...
    var items = ord.items.map(function (it, idx) {
      ...
      return {
        ...
        regraVencedora: r.regra || null, regraVencedoraLabel: r.regra ? (SKU_REGRA_LABEL[r.regra] || r.regra) : null,
        candidatos: r.candidatos || aliases,
  ...
  function openPedidoFicha360(orderId) {
    ...
    // §Correção CAIXA×FICHA (Parte 2): reorganização puramente VISUAL — nenhum valor recalculado.
    // Linha zero nunca renderiza; valores brutos de referência (comSubs/servicoBruta) nunca aparecem
    // inline (só na Auditoria técnica); a decomposição de Service Fee vira uma tabela de 2 colunas
    // em vez de texto corrido — só para fieldKey 'servico', que é onde essa decomposição existe.
    var taxaLine = function (r) {
      ...
      return '<div class="fin-line rowlink" title="Fonte: ' + esc(r.origem) + '"><span>' + esc(r.label) + '</span><b class="' + (r.valor < 0 ? 'neg' : r.valor > 0 ? 'pos' : '') + '">' + brlC(r.valor) + '</b></div>' + subHtml;
    };
    ...
    // Linhas com valor zero nunca renderizam (nem aqui, nem nas sub-tabelas) — a soma do grupo usa
    // sempre o grupo ORIGINAL (somaGrupo(g)), idêntica à do grupo filtrado (zero não muda soma).
    var grupoBlock = function (titulo, g, sempreMostra) {
      var gf = g.filter(function (r) { return r.valor; });
      if (!gf.length && !sempreMostra) return '';
      ...
      return (gf.length ? gf.map(taxaLine).join('') : '<span class="tag neutral">nenhum lançamento deste tipo neste pedido</span>') +
        (gf.length > 1 ? '<div class="fin-line" style="margin-top:2px"><span>Subtotal — ' + esc(titulo.toLowerCase()) + '</span><b class="' + (soma < 0 ? 'neg' : soma > 0 ? 'pos' : '') + '">' + brlC(soma) + '</b></div>' : '');
    };
    // 3. TAXAS SHOPEE DESCONTADAS DO VENDEDOR — só taxas reais, nomes reais, zero nunca aparece.
    var bloco3Taxas = '<div class="panel"><div class="ph"><h3>3. Taxas Shopee Descontadas do Vendedor</h3></div><div class="pb">' +
      (gTaxas.length ? grupoBlock('Taxas Shopee', gTaxas, true) : '<span class="tag neutral">sem taxas cobradas neste pedido</span>') +
      '</div></div>';
    ...
    if (gEnvio.length && envio && envio.subtotalC) {
      ...
      bloco4Envio = '<div class="panel"><div class="ph"><h3>4. Envio</h3></div><div class="pb">' +
        '<div class="fin-line"><span>Subtotal de Envio</span><b class="' + (envio.subtotalC < 0 ? 'neg' : 'pos') + '">' + brlC(envio.subtotalC) + '</b></div>' +
        '<details style="margin-top:2px"><summary style="cursor:pointer;font-size:13px;color:var(--muted,#666)">Ver composição</summary><div style="margin-top:4px">' + envioComps + '</div></details></div></div>';
    }
    // 5. AJUSTES QUE AFETARAM O REPASSE — PIX, Cupom, Reembolso, outros ajustes reais (nunca taxa).
    // Recolhido por padrão; só renderiza quando existe algum valor (§44-49 do complemento da Ficha).
    var gAjustes = gDescontos.concat(gOutros);
    var somaAjustes = somaGrupo(gDescontos) + somaGrupo(gOutros);
    var bloco5Ajustes = gAjustes.filter(function (r) { return r.valor; }).length ? '<div class="panel"><div class="ph"><h3>5. Ajustes que Afetaram o Repasse</h3></div><div class="pb">' +
      '<details><summary style="cursor:pointer;font-weight:600">Ver ajustes (' + brlC(somaAjustes) + ')</summary><div style="margin-top:6px">' + grupoBlock('Ajustes', gAjustes, true) + '</div></details>' +
      '</div></div>' : '';
    // 6. CRÉDITOS / COMPENSAÇÕES — só quando existem.
    var bloco6Creditos = gCreditos.filter(function (r) { return r.valor; }).length ? '<div class="panel"><div class="ph"><h3>6. Créditos / Compensações</h3></div><div class="pb">' + grupoBlock('Créditos', gCreditos, true) + '</div></div>' : '';
    // 7. CONCILIAÇÃO DO PAGAMENTO — equação explícita, substitui "Total (todos os grupos)". Usa
    // exatamente os mesmos grupos já somados (taxasSomaSemAjusteC == gDescontos+gEnvio+gTaxas+
    // gCreditos, validado empiricamente contra 1.662 pedidos reais) — nenhuma conta nova.
    ...
    var bloco7Conciliacao = mrRow ? '<div class="panel"><div class="ph"><h3>7. Conciliação do Pagamento</h3></div><div class="pb">' +
      fLine('Preço do produto', mrRow.preco) +
      (somaGrupo(gDescontos) ? fLine('Ajustes / reduções do pedido', somaGrupo(gDescontos)) : '') +
    ...
    // Cascata do Resultado do Pedido (Parte 2 da correção): parte do PAGAMENTO LIBERADO SHOPEE já
    // líquido de taxas/ajustes/envio/descontos (mrRow.liberado) — nunca subtrai Taxa de comissão/
    // Taxa de serviço de novo aqui (isso seria contar a mesma taxa duas vezes). Só desconta o que
    // ainda NÃO está dentro do Pagamento Liberado: custo do produto, custo de afiliados, Taxa
    // Acelera, ajustes POSTERIORES (Adjustment/gOutros) e devolução/perdas. A soma continua sendo
    // exatamente resultadoC (fórmula canônica, não recalculada) — só a apresentação mudou.
    var resultadoBlock = '<div class="panel"><div class="ph"><h3>Resultado do Pedido</h3></div><div class="pb">' +
      fLine('Venda', receitaC) +
      fLine('Pagamento Liberado Shopee', pagamentoLiberadoC, { total: true }) +
      ...
      (somaGrupo(gOutros) ? fLine('± Ajustes posteriores (Adjustment)', somaGrupo(gOutros)) : '') +
    ...
    // 7 blocos (Parte 2 da correção): 1.Resumo Financeiro (5 números) → 2.Produto e Venda →
    // 3.Taxas Shopee Descontadas do Vendedor → 4.Envio → 5.Ajustes que Afetaram o Repasse →
  ...
  // nasce classificada por walletCat() (abaixo); aqui só interpreta o que é "saiu para o banco".
  function wIsTransferenciaBancaria(t) { var cat = wEffCat(t); return cat === 'PIX' || cat === 'SAQUE' || cat === 'TRANSFERENCIA_BANCARIA'; }
  ...
  // §26/§53 do prompt "Correção Financeira": BUG DE DATA real — parseWalletDate() acima grava
  // t.date como um instante UTC (toISOString(), com 'Z'). t.date.slice(0,10) usado nos motores de
  // Caixa/Carteira para agrupar por dia extraía o DIA EM UTC, enquanto dbr(t.date) — usado para
  // EXIBIR essa mesma data — reconstrói o dia em horário LOCAL do navegador (toLocaleDateString).
  // Um Pix/ajuste tarde da noite (ex.: 22h-23h59 local) cruza a meia-noite ao virar UTC e passava a
  // ser agrupado no dia SEGUINTE ao que aparecia na tela — exatamente o sintoma relatado (fechamento
  // de um dia mostrando transferência com a data do dia anterior). Corrige na FONTE do agrupamento —
  // nunca no parser (walletKey/dedup por id permanece intocado, sem risco de duplicar reimportação) —
  // usando os mesmos getters LOCAIS que dbr() já usa para exibir, então agrupamento e exibição nunca
  // mais divergem.
  function walletLocalDayKey(t) {
  ...
  function walletCat(tipo, desc, amount) {
    ...
    // §F do complemento: nomenclaturas equivalentes a Pix/saque que não contêm literalmente essas
    // palavras — mesma natureza (Carteira → Banco), nunca despesa/taxa.
    if (/transferencia banc|transferencia para conta|transferencia bancaria/.test(t) || /transferencia banc|transferencia para conta|transferencia bancaria/.test(s)) return 'TRANSFERENCIA_BANCARIA';
    return 'OUTRO';
  }
  ...
  function walletMetrics() {
    ...
    var adjDays = {}; wallet.forEach(function (t) { if (t.adjust != null && Math.abs(t.adjust) > 0.01 && t.date) adjDays[walletLocalDayKey(t)] = 1; });
  ...
  function walletSaldoSeries() {
    var byDay = {}; wallet.filter(function (t) { return t.date && inPeriod(t.date); }).sort(function (a, b) { return a.date.localeCompare(b.date); }).forEach(function (t) { var d = walletLocalDayKey(t); if (!byDay[d]) byDay[d] = { label: monthDayLabel(d), a: null, b: null }; if (t.balance != null) byDay[d].a = t.balance; if (t.adjust != null) byDay[d].b = t.adjust; });
  ...
  function walletDayMetrics(dateKey) {
    var txs = wallet.filter(function (t) { return t.date && walletLocalDayKey(t) === dateKey && t.origin === 'SHOPEE'; });
  ...
  function walletDayAcelera(dateKey) {
    ...
    if (!d) return { n: 0 };
    var walletAcelera = wallet.filter(function (t) { return t.date && walletLocalDayKey(t) === dateKey && wEffCat(t) === 'ACELERA'; });
  ...
  function walletDayConciliacao(dateKey) {
    ...
    var real = rec.txs.filter(function (t) { return t.date && walletLocalDayKey(t) === dateKey && t.origin === 'SHOPEE'; });
    var g = walletOrigemDiag({ real: real });
    var diffsDay = rec.diffs.filter(function (d) { return d.date && walletLocalDayKey(d) === dateKey; });
    ...
    g.dup.forEach(function (d) { d.itens.forEach(function (t) { if (t.date && walletLocalDayKey(t) === dateKey) add(t, 'DUPLICIDADE', 'Possível cobrança duplicada — ' + d.fonte, nn(d.itens.length) + '× o mesmo valor (' + brl(d.valor) + ') na carteira'); }); });
  ...
  function importAcelera(file) {
    ...
    return file.arrayBuffer().then(function (ab) {
      ...
      parsed.rows.forEach(function (row) {
        // §BP do complemento: a chave já era preparada para multi-loja (prefixo antes fixo em
        // "default") — agora usa a operação ativa de verdade, nunca misturando pedido/resgate iguais
        // de operações diferentes. .id é opaco (nunca lido fora do dedupe daqui — sempre .resgate/.pedido).
        var id = opId + '|' + row.resgate + '|' + row.pedido; var ex = byId[id];
        if (!ex) { var rec = Object.assign({ id: id, operationId: opId, storeId: opId, firstImportAt: importedAt, lastImportAt: importedAt, fileName: file.name, history: [] }, row); byId[id] = rec; changed.push(rec); novo++; return; }
  ...
  // §Correção CAIXA×ACELERA — função ÚNICA e canônica do fechamento do dia do Acelera. O Caixa
  // NUNCA reconstrói "quais pedidos são do Acelera nesse dia" nem recalcula bruto/taxa/líquido —
  // só lê o que esta função devolve ("evita duas verdades"). Não recalcula/reagrupa nada do
  // Acelera (aceleraByResgate/aceleraByDia continuam intocados, MAIOR DATA DAS LINHAS DO MESMO
  // RESGATE preservada); só empacota o resultado do dia + cruza com os motores JÁ APROVADOS de
  // Expedição (expedidoSet/caixaDayExpedicao/pendenciasConferencia), nunca reinterpretando-os.
  function aceleraResultadoFechamento(dateKey) {
    var resgatesDia = aceleraByResgate(acelera).filter(function (g) { return g.data === dateKey; });
    var recsByResgate = {}; acelera.forEach(function (r) { (recsByResgate[r.resgate] = recsByResgate[r.resgate] || []).push(r); });
    var pedidosSet = {}; var pedidoBrutoHoje = {}; var pedidoValores = {};
    var resgates = resgatesDia.map(function (g) {
      var linhas = recsByResgate[g.resgate] || [];
      var pedidos = linhas.map(function (r) { return r.pedido; }).filter(function (v, i, a) { return a.indexOf(v) === i; });
      pedidos.forEach(function (p) { pedidosSet[p] = 1; });
      linhas.forEach(function (r) {
        pedidoBrutoHoje[r.pedido] = (pedidoBrutoHoje[r.pedido] || 0) + r.antecipado;
        var pv = pedidoValores[r.pedido] = pedidoValores[r.pedido] || { bruto: 0, taxa: 0, resgate: g.resgate };
        pv.bruto += r.antecipado; pv.taxa += r.taxa; pv.liquido = pv.bruto - pv.taxa;
      });
      var statusCounts = {}; linhas.forEach(function (r) { var lab = acStatusLabel(r.status); statusCounts[lab] = (statusCounts[lab] || 0) + 1; });
      return { resgate: g.resgate, n: g.n, pedidos: pedidos, bruto: g.antec, taxa: g.taxa, liquido: g.liquidoCalc, recebidoInformado: g.receb, divergente: g.divergente, aliqLabel: g.aliqLabel, statusCounts: statusCounts };
    });
    var pedidosAcelera = Object.keys(pedidosSet);
    var valorBruto = resgates.reduce(function (s, r) { return s + r.bruto; }, 0);
    var taxaAcelera = resgates.reduce(function (s, r) { return s + r.taxa; }, 0);
    var valorLiquido = resgates.reduce(function (s, r) { return s + r.liquido; }, 0);
    var exp = expedidoSet(); // motor já aprovado de Expedição — nunca recriado aqui
    var acByOrder = {}; acelera.forEach(function (r) { acByOrder[r.pedido] = true; });
    // §7/§17/§31 do complemento: dentro do MESMO resgate consolidado do dia, separa (só informativo —
    // nunca recalcula bruto/taxa/líquido do resgate) quais pedidos foram expedidos HOJE (novos) de
    // quais foram expedidos em dia ANTERIOR e só chegaram no Acelera hoje (recebimento de pendência
    // anterior). Nunca cria uma segunda venda nem duplica faturamento — é só rótulo de origem.
    var pedidosAntigosRecebidosHoje = pedidosAcelera.filter(function (p) { var info = exp[p]; return info && info.at && info.at.slice(0, 10) < dateKey; });
    var valorAntigosRecebidosHoje = pedidosAntigosRecebidosHoje.reduce(function (s, p) { return s + (pedidoBrutoHoje[p] || 0); }, 0);
    // Direção 1: pedidos do ACELERA neste dia sem confirmação de expedição (bipe/Full) — pergunta
    // "quem o Acelera confirma que não sabemos ter saído?".
    var aceleraSemConfirmacaoExpedicao = pedidosAcelera.filter(function (p) { return !exp[p]; });
    // Direção 2: pedidos ESPERADOS para conferência de expedição neste dia (sessão operacional do
    // dia; sem sessão, cai no fallback já usado por caixaDayExpedicao) — cruza com o Acelera (em
    // qualquer data, pois a antecipação pode chegar depois do envio).
    var diaExp = caixaDayExpedicao(dateKey); // motor já aprovado — nunca recriado aqui
    var expedidosEsperados = diaExp.temSessao ? diaExp.itens.map(function (i) { return i.orderId; }) : Object.keys(exp).filter(function (oid) { var info = exp[oid]; return info && info.at && info.at.slice(0, 10) === dateKey; });
    var expedidosEncontrados = expedidosEsperados.filter(function (p) { return !!acByOrder[p]; });
    var expedidosNaoEncontrados = expedidosEsperados.filter(function (p) { return !acByOrder[p]; });
    var tocados = {}; pedidosAcelera.forEach(function (p) { tocados[p] = 1; }); expedidosEsperados.forEach(function (p) { tocados[p] = 1; });
    var pendencias = pendenciasConferencia().filter(function (i) { return ['OK', 'RESOLVIDO'].indexOf(i.situacao) < 0 && tocados[i.orderId]; }); // motor já aprovado — só filtra pelo dia
    var divergencias = resgates.filter(function (r) { return r.divergente; }).map(function (r) { return { resgate: r.resgate, tipo: 'LIQUIDO_DIVERGENTE', motivo: 'Líquido informado pela Shopee diverge do calculado (antecipado − taxa) além da tolerância de 2 centavos.', diffC: r.recebidoInformado - r.liquido }; });
    return {
      dateKey: dateKey,
      resgates: resgates, resgatesN: resgates.length,
      pedidosAcelera: pedidosAcelera, pedidosAceleraN: pedidosAcelera.length,
      expedidosEsperados: expedidosEsperados, expedidosEsperadosN: expedidosEsperados.length,
      expedidosEncontrados: expedidosEncontrados, expedidosEncontradosN: expedidosEncontrados.length,
      expedidosNaoEncontrados: expedidosNaoEncontrados,
      aceleraSemConfirmacaoExpedicao: aceleraSemConfirmacaoExpedicao,
      valorBruto: valorBruto, taxaAcelera: taxaAcelera, valorLiquido: valorLiquido,
      pedidosAntigosRecebidosHoje: pedidosAntigosRecebidosHoje, valorAntigosRecebidosHoje: valorAntigosRecebidosHoje,
      pedidoValores: pedidoValores,
      divergencias: divergencias, pendencias: pendencias
    };
  }
  ...
  function importMinhaRenda(file) {
    ...
    return file.arrayBuffer().then(function (ab) {
      ...
      p.renda.forEach(function (row, i) { var id = row.orderId + '|' + row.ver + '|' + (row.sku || '') + '|' + (row.refundId || '') + '|' + i; if (!byR[id]) { var rec = Object.assign({ id: id, operationId: opId, fileName: file.name, importedAt: importedAt }, row); byR[id] = rec; chR.push(rec); stats.renda++; } });
      mrRenda = Object.values(byR);
      // Shipping (preserva investigação manual §21)
      var byS = {}; mrShip.forEach(function (x) { byS[x.id] = x; }); var chS = [];
      p.ship.forEach(function (row) { var id = row.orderId; var ex = byS[id]; if (!ex) { byS[id] = Object.assign({ id: id, operationId: opId, invStatus: 'NAO_ANALISADO', invNote: '', invResp: '', importedAt: importedAt, fileName: file.name }, row); chS.push(byS[id]); stats.ship++; } else { ex.esperado = row.esperado; ex.real = row.real; ex.motivo = row.motivo; chS.push(ex); } });
      mrShip = Object.values(byS);
      // Adjustment
      var byA = {}; mrAdj.forEach(function (x) { byA[x.id] = x; }); var chA = [];
      p.adj.forEach(function (row) { var id = (row.orderId || 'sem') + '|' + row.valor + '|' + row.seq; if (!byA[id]) { byA[id] = Object.assign({ id: id, operationId: opId, importedAt: importedAt }, row); chA.push(byA[id]); stats.adj++; } });
      mrAdj = Object.values(byA);
      // Service Fee
      var byV = {}; mrSvc.forEach(function (x) { byV[x.id] = x; }); var chV = [];
      p.svc.forEach(function (row) { var id = row.orderId + '|' + row.seq; if (!byV[id]) { byV[id] = Object.assign({ id: id, operationId: opId, importedAt: importedAt }, row); chV.push(byV[id]); stats.svc++; } });
  ...
  function makeProdutos(opts) {
    ...
    // §5/§6 do prompt "custo de Produtos não chega em Pedidos" — função canônica ÚNICA para atribuir
    // família a variações, usada por toda ação de classificação (individual, master ou em massa):
    // 1) localiza as variações; 2) atribui familyId em memória; 3) persiste TODAS em IndexedDB;
    // 4) aguarda a persistência antes de resolver; 5) dispara onChange()→rebuildSkuCost() (dentro de
    // saveVars). Nunca atribuir familyId "à mão" fora desta função — evita o bug de uma ação em massa
    // esquecer de persistir ou de reindexar alguma variação.
    function assignFamilyToVariations(variationIds, familyId) {
      var vs = variationIds.map(function (id) { return S2.variations.find(function (x) { return x.id === id; }); }).filter(Boolean);
      vs.forEach(function (v) { v.familyId = familyId || null; });
      return saveVars(vs).then(function () { return vs; });
    }
    ...
    function wireResults(masters) {
      ...
      // §8: o custo/família clicado na linha MASTER precisa alcançar TODAS as variações do produto
      // (m.all), nunca só as que estão visíveis sob o filtro/busca ativos no momento (m.shown) — senão
      // uma variação fora do filtro fica órfã de família mesmo depois de "classificar todas".
      appEl.querySelectorAll('[data-mcost]').forEach(function (c) { c.onclick = function () { var m = masters.find(function (x) { return x.p.id === c.dataset.mcost; }); openCostEditor(m ? m.all.map(function (v) { return v.id; }) : []); }; });
    }
    function editVarFamily(cell, vid) { var v = S2.variations.find(function (x) { return x.id === vid; }); if (!v) return; var actives = S2.families.filter(function (f) { return f.status === 'ACTIVE' || f.id === v.familyId; }); cell.innerHTML = '<select class="select sm inl"><option value="">— sem família —</option>' + actives.map(function (f) { return '<option value="' + f.id + '"' + (v.familyId === f.id ? ' selected' : '') + '>' + esc(f.name) + '</option>'; }).join('') + '<option value="__new">+ criar nova família…</option></select>'; var se = cell.querySelector('select'); se.focus(); se.onchange = function () { if (se.value === '__new') { familyEditor(null, function (f) { assignFamilyToVariations([v.id], f.id).then(function () { refresh(); toast('Família criada e aplicada', f.name); }); }); return; } assignFamilyToVariations([v.id], se.value || null).then(function () { refresh(); toast('Salvo', 'Família da variação atualizada.'); }); }; se.onkeydown = function (e) { if (e.key === 'Escape') refresh(); }; se.onblur = function () { setTimeout(function () { if (document.body.contains(se) && se.value !== '__new') refresh(); }, 150); }; }
    ...
    // §8/§24: ações da linha MASTER (família/preço em massa) precisam alcançar TODAS as variações do
    // produto (m.all) — nunca só m.shown (o subconjunto visível sob o filtro/busca ativos), senão uma
    ...
    function applyFamily(ids, fid) { assignFamilyToVariations(ids, fid).then(function () { S2.selected.clear(); S2.allFiltered = false; refresh(); toast('Família atribuída', ids.length + ' SKUs' + (fid ? ' em “' + famName(fid) + '”' : '') + '.'); }); }
    ...
    // "não ambíguo" (todas as variações classificadas do produto usam a MESMA família — reparável
    // automaticamente com um clique) e "ambíguo" (há mais de uma família entre as classificadas —
    // nunca decide sozinho, sempre exige escolha manual).
    function auditOrphans() {
      var out = { unambiguous: [], ambiguous: [] };
      S2.products.forEach(function (p) {
        var all = S2.varByProduct[p.id] || [];
        if (all.length < 2) return;
        var classified = all.filter(function (v) { return v.familyId; });
        var orphans = all.filter(function (v) { return !v.familyId; });
        if (!classified.length || !orphans.length) return;
        var fams = uniq(classified.map(function (v) { return v.familyId; }));
        if (fams.length === 1) out.unambiguous.push({ p: p, orphans: orphans, familyId: fams[0] });
        else out.ambiguous.push({ p: p, orphans: orphans, fams: fams });
      });
      return out;
    }
    function renderAuditoria() {
      ...
      var total = nUnamb + nAmb;
      function rowsFor(list, ambiguous) {
        var rows = [];
        list.forEach(function (g) {
          var famLabel = ambiguous ? g.fams.map(function (fid) { return famName(fid); }).join(' / ') : famName(g.familyId);
          g.orphans.forEach(function (v) {
            rows.push('<tr><td>' + esc(g.p.name) + '</td><td class="mono">' + esc(v.sku || '—') + '</td><td class="mono">' + esc(v.referenceSku || '—') + '</td><td>' + esc(v.variationName || '(única)') + '</td><td>' + esc(famLabel) + (ambiguous ? ' <span class="tag warn">múltiplas famílias</span>' : '') + '</td><td class="mono">' + (v.familyId ? esc(v.familyId) : '—') + '</td><td>' + (ambiguous ? '<span class="tag warn">requer escolha manual</span>' : '<span class="tag ok">reparável automaticamente</span>') + '</td></tr>');
          });
        });
        return rows.join('');
      }
      appEl.innerHTML = head('Variações sem família em produtos que já têm classificação — mostra exatamente quais SKUs ficaram órfãos e por quê.') + tabsHtml('auditoria') +
        ...
        (total === 0 ? '<span class="tag ok">🟢 Nenhuma variação órfã encontrada — todo produto com família cadastrada tem todas as suas variações vinculadas.</span>' :
          ...
          (audit.unambiguous.length ? '<button class="btn-sm primary" id="repairAll" style="margin-bottom:10px">Reparar automaticamente (' + nn(nUnamb) + ' caso(s) não ambíguo(s))</button>' : '') +
          '<div class="table-wrap"><table><thead><tr><th>Produto</th><th>SKU</th><th>Reference SKU</th><th>Variação</th><th>Família (das irmãs classificadas)</th><th>familyId atual</th><th>Situação</th></tr></thead><tbody>' +
          rowsFor(audit.unambiguous, false) + rowsFor(audit.ambiguous, true) +
          '</tbody></table></div>')) +
        '</div></div>';
      wireTabs();
      var btn = q('#repairAll');
      if (btn) btn.onclick = function () {
        Promise.all(audit.unambiguous.map(function (g) { return assignFamilyToVariations(g.orphans.map(function (v) { return v.id; }), g.familyId); }))
          .then(function () { render(); toast('Reparo concluído', nUnamb + ' variação(ões) vinculada(s) à família já usada pelas SKUs irmãs do mesmo produto.'); });
      };
    }
  ...
  // §AA/§AC do complemento: FONTE ÚNICA de Receita Bruta — Resumo do dia e DRE precisam mostrar
  // exatamente o mesmo número quando falam do mesmo conceito. Usa sempre pedidoComposicaoFinanceira
  // (mrRow.preco quando há Income cruzado; senão o valor do próprio Pedido) — nunca o.totalAmount
  // direto, que podia divergir silenciosamente do que a DRE já usava.
  function caixaReceitaBrutaPedido(o) { var c = pedidoComposicaoFinanceira(o.id); return c.receitaC != null ? c.receitaC / 100 : (o.totalAmount || 0); }
  ...
  function caixaDayVendas(dateKey) {
    ...
    pagas.forEach(function (o) { var c = pedidoComposicaoFinanceira(o.id); valor += (c.receitaC != null ? c.receitaC / 100 : (o.totalAmount || 0)); if (c.resultadoC != null) { lucro += c.resultadoC; nLucroConhecido++; } });
  ...
  function caixaDayMovimentosCarteira(dateKey) {
    var txs = wallet.filter(function (t) { return t.origin === 'SHOPEE' && t.date && walletLocalDayKey(t) === dateKey; });
  ...
  // §E-H/§BT do complemento: "Transferido para banco" NUNCA pode depender só de lançamento manual —
  // Pix/Saque/Transferência bancária real JÁ importados da Carteira contam automaticamente. Entradas
  // manuais (bankTransfers) continuam existindo (histórico/contas sem Carteira importada), mas nunca
  // duplicam silenciosamente um Pix já reconhecido — só sinalizam possível duplicidade (mesmo dia,
  // valor ~igual) para o operador decidir; nunca some sozinho.
  function caixaDayTransferenciasBancarias(dateKey) {
    var autoTxs = wallet.filter(function (t) { return t.origin === 'SHOPEE' && t.date && walletLocalDayKey(t) === dateKey && wIsTransferenciaBancaria(t); });
    var autoTotal = autoTxs.reduce(function (s, t) { return s + Math.abs(t.amount); }, 0);
    var manual = bankTransfers.filter(function (t) { return t.quando && t.quando.slice(0, 10) === dateKey; });
    var manualTotal = manual.reduce(function (s, t) { return s + t.valor; }, 0);
    var manualComDuplicidade = manual.map(function (m) { return { t: m, possivelDuplicado: autoTxs.some(function (a) { return Math.abs(Math.abs(a.amount) - m.valor) <= 1; }) }; });
    return { autoTxs: autoTxs, autoTotal: r2(autoTotal), manual: manual, manualTotal: r2(manualTotal), manualComDuplicidade: manualComDuplicidade, total: r2(autoTotal + manualTotal) };
  }
  // §O/§P do complemento: CONCILIAÇÃO DO DINHEIRO — equação explícita a partir do saldo real da
  // Carteira (campo `balance` que já vem por linha na planilha importada, quando presente). Nunca
  // inventa saldo inicial: se não há linha anterior com saldo conhecido, mostra "sem saldo inicial
  // conhecido" em vez de assumir zero.
  function caixaDaySaldoCarteira(dateKey) {
    var antes = wallet.filter(function (t) { return t.balance != null && t.date && walletLocalDayKey(t) < dateKey; }).sort(function (a, b) { return (a.date || '').localeCompare(b.date || ''); });
    var saldoInicial = antes.length ? antes[antes.length - 1].balance : null;
    var doDia = wallet.filter(function (t) { return t.balance != null && t.date && walletLocalDayKey(t) === dateKey; }).sort(function (a, b) { return (a.date || '').localeCompare(b.date || ''); });
    var saldoFinalInformado = doDia.length ? doDia[doDia.length - 1].balance : null;
    var mov = caixaDayMovimentosCarteira(dateKey);
    var entradas = mov.creditos.reduce(function (s, t) { return s + t.amount; }, 0);
    var debitosOperacionais = mov.debitos.filter(function (t) { return !wIsTransferenciaBancaria(t); }).reduce(function (s, t) { return s + Math.abs(t.amount); }, 0);
    var transferenciasBancarias = mov.debitos.filter(function (t) { return wIsTransferenciaBancaria(t); }).reduce(function (s, t) { return s + Math.abs(t.amount); }, 0);
    var saldoFinalCalculado = saldoInicial != null ? r2(saldoInicial + entradas - debitosOperacionais - transferenciasBancarias) : null;
    var diferenca = (saldoFinalCalculado != null && saldoFinalInformado != null) ? r2(saldoFinalInformado - saldoFinalCalculado) : null;
    return { saldoInicial: saldoInicial, saldoFinalInformado: saldoFinalInformado, entradas: r2(entradas), debitosOperacionais: r2(debitosOperacionais), transferenciasBancarias: r2(transferenciasBancarias), saldoFinalCalculado: saldoFinalCalculado, diferenca: diferenca };
  }
  // §K/§L/§AG/§AH do complemento: MOVIMENTOS APÓS O RECEBIMENTO, agrupados por NATUREZA (nunca um
  // "Ajustes" genérico escondendo a origem). O crédito que É o próprio resgate do dia (mostrado no
  // bloco 2) nunca entra aqui de novo — evita duas verdades para o mesmo evento.
  var CAIXA_MOV_CAT_LABEL = { AJUSTES_ACELERA: 'Ajustes Shopee Acelera', DEVOLUCOES_REEMBOLSOS: 'Devoluções / Reembolsos', AJUSTES_SHOPEE: 'Ajustes Shopee', CREDITOS_COMPENSACOES: 'Créditos / Compensações', OUTROS_DEBITOS: 'Outros débitos', OUTROS_CREDITOS: 'Outros créditos', TRANSFERENCIAS_BANCARIAS: 'Transferências Bancárias', NAO_IDENTIFICADOS: 'Não identificados' };
  function caixaMovCategoria(t, ac) {
    if (wIsTransferenciaBancaria(t)) return 'TRANSFERENCIAS_BANCARIAS';
    var cat = wEffCat(t);
    if (cat === 'ACELERA') {
      if (t.amount > 0) { var resgId = wCreditResgateId(t); if (resgId && ac && ac.resgates.some(function (r) { return r.resgate === resgId; })) return null; }
      return 'AJUSTES_ACELERA';
    }
    if (cat === 'DEVOLUCAO' || cat === 'CANCELAMENTO') return 'DEVOLUCOES_REEMBOLSOS';
    if (cat === 'COMPENSACAO' || cat === 'INDENIZACAO') return 'CREDITOS_COMPENSACOES';
    if (cat === 'OUTRO' || cat === 'SEMLINHA') return 'NAO_IDENTIFICADOS';
    return t.amount < 0 ? 'AJUSTES_SHOPEE' : 'OUTROS_CREDITOS';
  }
  function caixaDayMovimentosPosteriores(dateKey) {
    var ac = aceleraResultadoFechamento(dateKey);
    var mov = caixaDayMovimentosCarteira(dateKey);
    var todos = mov.creditos.concat(mov.debitos);
    var porCategoria = {};
    todos.forEach(function (t) {
      var k = caixaMovCategoria(t, ac); if (!k) return;
      var g = porCategoria[k] = porCategoria[k] || { categoria: k, label: CAIXA_MOV_CAT_LABEL[k], n: 0, valor: 0, itens: [] };
      g.n++; g.valor += t.amount; g.itens.push(t);
    });
    return { porCategoria: porCategoria, todos: todos.filter(function (t) { return caixaMovCategoria(t, ac); }) };
  }
  ...
  // §1/§3/§25/§26 do complemento CAIXA×CARTEIRA: "resolver" uma pendência da Carteira nunca apaga o
  // lançamento — muda seu estado de "sem explicação" para "explicado". Uma vez que o operador
  // classifica/resolve (internalStatus EXPLICADO ou RESOLVIDO, salvo em wsetCls/walletcls — nunca
  // reescrito aqui), o lançamento some de Pendências e passa a aparecer em Lançamentos Conciliados
  // (caixaDayLancamentosConciliados), preservando o histórico completo (c.history) para sempre.
  function wCarteiraResolvido(c) { return !!c && (c.internalStatus === 'RESOLVIDO' || c.internalStatus === 'EXPLICADO'); }
  function caixaDayPendenciasCarteira(dateKey) {
    ...
    txs.forEach(function (t) {
      ...
      if (wCarteiraResolvido(c)) return; // já classificado/resolvido — vira Lançamento Conciliado, não pendência
  ...
  // §2/§18/§22/§24 do complemento: Lançamentos Conciliados da Carteira — tudo que JÁ foi explicado
  // (automático: walletOrigin bateu 🟢; ou manual: operador classificou/resolveu), categorizado para
  // nunca cair num "Outras Receitas" genérico que esconde a origem. §12: o crédito que É o espelho do
  // líquido do Acelera do dia (mesmo pedido/resgate de hoje) NUNCA conta como "outra receita" — ele
  // é a confirmação de que o Acelera chegou na Carteira, mostrado à parte na comparação do bloco 3.
  function caixaCartCategoria(t, ac) {
    var cat = wEffCat(t); var oid = wOrderId(t); var resgId = wCreditResgateId(t);
    if (t.amount < 0) return 'DEBITO';
    if (cat === 'ACELERA' && ac && resgId && ac.resgates.some(function (r) { return r.resgate === resgId; })) return 'ACELERA_DIA';
    if (oid && ac && ac.pedidosAntigosRecebidosHoje && ac.pedidosAntigosRecebidosHoje.indexOf(oid) >= 0) return 'PENDENCIA_ANTERIOR';
    if (cat === 'COMPENSACAO' || cat === 'INDENIZACAO') return 'COMPENSACAO';
    return 'OUTRAS_RECEITAS';
  }
  var CAIXA_CART_CAT_LABEL = { ACELERA_DIA: 'Acelera do dia', PENDENCIA_ANTERIOR: 'Recebimento de pendência anterior', COMPENSACAO: 'Compensação / Recuperação', OUTRAS_RECEITAS: 'Outras receitas', DEBITO: 'Débito da Carteira' };
  function caixaDayLancamentosConciliados(dateKey) {
    if (!wallet.length) return { itens: [], porCategoria: {} };
    var ac = aceleraResultadoFechamento(dateKey);
    var txs = wallet.filter(function (t) { return t.origin === 'SHOPEE' && t.date && walletLocalDayKey(t) === dateKey; });
    var itens = [];
    txs.forEach(function (t) {
      var cat = wEffCat(t); var c = wgetCls(t.id) || {};
      var resolvidoManual = wCarteiraResolvido(c);
      var o = WALLET_ORIGEM_CATS[cat] ? walletOrigin(t) : null;
      var conciliadoAuto = o && o.confidence === 'alta' && o.tier === 'ID exato' && o.ok;
      if (!resolvidoManual && !conciliadoAuto) return; // continua pendência (ou fora de cobertura — não é lançamento explicado)
      itens.push({ t: t, categoria: caixaCartCategoria(t, ac), origem: o ? o.fonte : (c.catManual ? 'Classificação manual — ' + wcatLabel(c.catManual) : wcatLabel(cat)), viaClassificacaoManual: resolvidoManual, viaOrigemAutomatica: !!conciliadoAuto });
    });
    var porCategoria = {};
    itens.forEach(function (it) { var k = it.categoria; var g = porCategoria[k] = porCategoria[k] || { categoria: k, label: CAIXA_CART_CAT_LABEL[k], n: 0, valor: 0, itens: [] }; g.n++; g.valor += it.t.amount; g.itens.push(it); });
    return { itens: itens, porCategoria: porCategoria };
  }
  ...
  // §31 do prompt "Correção Financeira": conta bancária precisa saber a qual EMPRESA (CNPJ) pertence
  // — nunca mostrar conta de CNPJ errado por padrão. companyId vem da empresa da operação ativa no
  ...
  function bankAccountsForActiveCompany() {
    var op = opActive(); var comp = op ? opCompany(op) : null; var compId = comp ? comp.id : null;
    return bankAccounts.filter(function (b) { return !b.companyId || b.companyId === compId; });
  }
  // §W-Y do complemento: cadastrar conta DIRETO no fluxo do fechamento — nunca obrigar o operador a
  // sair da tela. Mesmo bankAccountSave/store já aprovado (§100-101); só um modal mais enxuto.
  function openCadastrarContaModal(onSaved) {
    var d = document.createElement('div'); d.className = 'drawer'; var panel = document.createElement('div'); panel.className = 'drawer-panel'; panel.style.width = '480px'; panel.style.maxWidth = '96vw';
    d.appendChild(panel); d.onclick = function (e) { if (e.target === d) d.remove(); }; document.body.appendChild(d);
    panel.innerHTML = '<div class="dh"><div><b>Cadastrar nova conta bancária</b></div><button class="x">&times;</button></div><div class="dbd">' +
      '<label class="fld">Nome/apelido da conta</label><input class="input" id="nc-nome" style="width:100%" placeholder="Ex.: Itaú Líder">' +
      '<label class="fld">Banco</label><input class="input" id="nc-banco" style="width:100%">' +
      '<div style="display:flex;gap:8px"><div style="flex:1"><label class="fld">Agência</label><input class="input" id="nc-agencia" style="width:100%"></div><div style="flex:1"><label class="fld">Conta</label><input class="input" id="nc-conta" style="width:100%"></div></div>' +
  ...
  // §BK/§BL do complemento — migração idempotente: só cria a operação padrão e estampa dado legado
  // na PRIMEIRA vez (quando ainda não existe nenhuma operação cadastrada). Rodadas seguintes do boot
  // encontram `operations.length` > 0 e não fazem nada — nunca reescreve/duplica.
  function opRunMigrationIfNeeded(raw) {
    if (operations.length) return Promise.resolve();
    var now = new Date().toISOString();
    var company = { id: 'CO-default', razaoSocial: 'Líder Molduras LTDA', nomeFantasia: 'Líder Molduras', cnpj: '', inscricaoEstadual: '', observacao: '', ativa: true, createdAt: now };
    var operation = { id: 'OP-default', companyId: company.id, nome: 'Líder Molduras · Shopee', marketplace: 'Shopee', storeName: 'lidermolduras', sellerId: '', ativa: true, createdAt: now };
    companies = [company]; operations = [operation];
    var stampsToSave = [];
    ['orders', 'occ', 'wallet', 'affConv', 'affRpa', 'affVb', 'mrRenda', 'mrShip', 'mrAdj', 'mrSvc'].forEach(function (key) {
    ...
    // Acelera é o único caso com re-chaveamento seguro (id sempre foi sintético/opaco, nunca lido
    // fora do dedupe do próprio importAcelera — troca o prefixo "default|" pelo id real da operação).
    var acArr = raw.acelera || []; var acChanged = [];
    acArr.forEach(function (rec) {
      if (rec.operationId) return;
      rec.operationId = operation.id; rec.storeId = operation.id;
      if (typeof rec.id === 'string' && rec.id.indexOf('default|') === 0) rec.id = operation.id + '|' + rec.id.slice('default|'.length);
      acChanged.push(rec);
    });
  ...
  var STORE_BY_ARRKEY = { orders: 'orders', occ: 'occ', wallet: 'wallet', affConv: 'affconv', affRpa: 'affrpa', affVb: 'affvb', mrRenda: 'mrrenda', mrShip: 'mrship', mrAdj: 'mradj', mrSvc: 'mrsvc' };
  ...
  function opUpdateSelectorLabel() {
    ...
    if (activeOperationId === OP_ALL) { el.textContent = 'Todas as Operações'; return; }
    var op = opActive(); el.textContent = op ? opLabel(op) : '—';
  }
  function opSetActive(id) {
    activeOperationId = id;
    return putMany('settings', [{ id: 'activeOperationId', data: id }]).then(function () { return location.reload(); });
  }
  // Recarrega a página após trocar de operação — o boot inteiro (filtro em memória de orders/wallet/
  // acelera/mrRenda/occ/affConv/...) depende de rodar do zero; trocar em memória sem reload arriscaria
  // misturar dados de duas operações nos motores que já leram os arrays antigos.
  function openOperationSelector() {
    ...
    d.appendChild(panel); d.onclick = function (e) { if (e.target === d) d.remove(); }; document.body.appendChild(d);
    var rows = operations.filter(function (o) { return o.ativa !== false; }).map(function (o) {
      var sel = o.id === activeOperationId;
      return '<div class="fin-line rowlink" data-opsel="' + esc(o.id) + '" style="cursor:pointer' + (sel ? ';font-weight:700' : '') + '"><span>' + (sel ? '🟢 ' : '') + esc(opLabel(o)) + '</span></div>';
    }).join('');
    panel.innerHTML = '<div class="dh"><div><b>Empresa / Operação</b></div><button class="x">&times;</button></div><div class="dbd">' +
      '<div class="fin-line rowlink" data-opsel="' + OP_ALL + '" style="cursor:pointer' + (activeOperationId === OP_ALL ? ';font-weight:700' : '') + '"><span>' + (activeOperationId === OP_ALL ? '🟢 ' : '') + 'Todas as Operações <span class="footnote" style="margin:0">(visão consolidada — nunca mistura o dado gravado)</span></span></div>' +
      '<div style="margin:10px 0 4px" class="footnote">Operações</div>' + (rows || '<div class="footnote">Nenhuma operação cadastrada.</div>') +
      '<div style="margin-top:14px;display:flex;gap:8px"><button class="btn-sm primary" id="op-nova">+ Nova operação</button><button class="btn-sm" id="op-gerenciar">Gerenciar operações</button></div>' +
      '</div>';
    panel.querySelector('.x').onclick = function () { d.remove(); };
    panel.querySelectorAll('[data-opsel]').forEach(function (b) { b.onclick = function () { var id = b.dataset.opsel; d.remove(); if (id !== activeOperationId) opSetActive(id); }; });
    panel.querySelector('#op-nova').onclick = function () { d.remove(); openNovaOperacaoModal(); };
    panel.querySelector('#op-gerenciar').onclick = function () { d.remove(); openGerenciarOperacoes(); };
  }
  function companySave(rec) {
    var id = rec.id || ('CO-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8));
    var full = { id: id, razaoSocial: rec.razaoSocial, nomeFantasia: rec.nomeFantasia || rec.razaoSocial, cnpj: rec.cnpj || '', inscricaoEstadual: rec.inscricaoEstadual || '', observacao: rec.observacao || '', ativa: rec.ativa !== false, createdAt: rec.createdAt || new Date().toISOString() };
    companies = [full].concat(companies.filter(function (c) { return c.id !== id; }));
    return putMany('companies', [full]).then(function () { return full; });
  }
  function operationSave(rec) {
    var id = rec.id || ('OP-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8));
    var full = { id: id, companyId: rec.companyId, nome: rec.nome, marketplace: rec.marketplace || 'Shopee', storeName: rec.storeName || '', sellerId: rec.sellerId || '', ativa: rec.ativa !== false, createdAt: rec.createdAt || new Date().toISOString() };
    operations = [full].concat(operations.filter(function (o) { return o.id !== id; }));
    return putMany('operations', [full]).then(function () { return full; });
  }
  // §BA-BD do complemento: wizard simplificado (empresa existente ou nova + marketplace + loja).
  // §Bug Nova Empresa (prompt mestre Parte 1): a visibilidade dos campos de empresa nova precisa
  // depender SÓ do valor atual do select (#no-empresa), nunca de companies.length — antes disso, com
  // companies.length > 0 o container nascia display:none mesmo com "+ Nova empresa" pré-selecionado
  // (é sempre a primeira <option>), então a validação de "razão social" travava num campo invisível.
  // syncNovaEmpresaFields() é a ÚNICA fonte de verdade da visibilidade, chamada tanto no render inicial
  // quanto no onchange — nunca mais um cálculo derivado de outra coisa que não o valor do select.
  function cnpjDigits(v) { return String(v || '').replace(/\D/g, ''); }
  function openNovaOperacaoModal() {
    var d = document.createElement('div'); d.className = 'drawer'; var panel = document.createElement('div'); panel.className = 'drawer-panel'; panel.style.width = '480px'; panel.style.maxWidth = '96vw';
    d.appendChild(panel); d.onclick = function (e) { if (e.target === d) d.remove(); }; document.body.appendChild(d);
    function render2() {
      var companyOptions = '<option value="__nova__">+ Nova empresa</option>' + companies.map(function (c) { return '<option value="' + esc(c.id) + '">' + esc(c.nomeFantasia || c.razaoSocial) + (c.cnpj ? ' — ' + esc(c.cnpj) : '') + '</option>'; }).join('');
      panel.innerHTML = '<div class="dh"><div><b>Nova operação</b></div><button class="x">&times;</button></div><div class="dbd">' +
        '<label class="fld">Empresa (CNPJ)</label><select class="select" id="no-empresa" style="width:100%">' + companyOptions + '</select>' +
        '<div id="no-empresa-nova">' +
        '<label class="fld">Razão social *</label><input class="input" id="no-razao" style="width:100%">' +
        '<label class="fld">Nome fantasia *</label><input class="input" id="no-fantasia" style="width:100%">' +
        '<label class="fld">CNPJ *</label><input class="input" id="no-cnpj" style="width:100%" placeholder="00.000.000/0000-00">' +
        '<label class="fld">Inscrição estadual</label><input class="input" id="no-ie" style="width:100%">' +
        '<label class="fld">Observação</label><input class="input" id="no-obs" style="width:100%">' +
        '<div id="no-err"></div>' +
        '</div>' +
        '<label class="fld">Marketplace</label><select class="select" id="no-mkt" style="width:100%"><option value="Shopee">Shopee</option><option value="Mercado Livre">Mercado Livre</option><option value="Amazon">Amazon</option><option value="Outro">Outro</option></select>' +
        '<label class="fld">Nome da loja</label><input class="input" id="no-loja" style="width:100%" placeholder="Ex.: lidermolduras">' +
        '<label class="fld">Seller ID (opcional)</label><input class="input" id="no-seller" style="width:100%">' +
        '<div style="margin-top:12px"><button class="btn-sm primary" id="no-salvar">Salvar operação</button></div>' +
        '</div>';
      panel.querySelector('.x').onclick = function () { d.remove(); };
      var empresaSel = panel.querySelector('#no-empresa'); var novaDiv = panel.querySelector('#no-empresa-nova');
      function syncNovaEmpresaFields() { novaDiv.style.display = empresaSel.value === '__nova__' ? 'block' : 'none'; }
      syncNovaEmpresaFields(); // estado inicial — nunca companies.length
      empresaSel.onchange = syncNovaEmpresaFields;
      panel.querySelector('#no-salvar').onclick = function () {
        var loja = panel.querySelector('#no-loja').value.trim(); if (!loja) { toast('Informe o nome da loja', '', true); return; }
        var proceed = function (companyId) {
          operationSave({ companyId: companyId, nome: (companies.find(function (c) { return c.id === companyId; }) || {}).nomeFantasia + ' · ' + panel.querySelector('#no-mkt').value, marketplace: panel.querySelector('#no-mkt').value, storeName: loja, sellerId: panel.querySelector('#no-seller').value.trim() }).then(function (op) {
            d.remove(); toast('Operação cadastrada', opLabel(op)); opSetActive(op.id);
          });
        };
        if (empresaSel.value === '__nova__') {
          var errEl = panel.querySelector('#no-err');
          var razao = panel.querySelector('#no-razao').value.trim();
          if (!razao) { errEl.innerHTML = '<div class="form-err">Informe a razão social da nova empresa.</div>'; return; }
          var cnpjRaw = panel.querySelector('#no-cnpj').value.trim();
          var digits = cnpjDigits(cnpjRaw);
          if (cnpjRaw && digits.length !== 14) { errEl.innerHTML = '<div class="form-err">CNPJ inválido — informe os 14 dígitos (ou deixe em branco).</div>'; return; }
          errEl.innerHTML = '';
          companySave({ razaoSocial: razao, nomeFantasia: panel.querySelector('#no-fantasia').value.trim() || razao, cnpj: cnpjRaw, inscricaoEstadual: panel.querySelector('#no-ie').value.trim(), observacao: panel.querySelector('#no-obs').value.trim() }).then(function (c) { proceed(c.id); });
        } else proceed(empresaSel.value);
      };
    }
    render2();
  }
  // §Y do complemento: editar/inativar/reativar — nunca excluir fisicamente uma operação com histórico.
  function openGerenciarOperacoes() {
    var d = document.createElement('div'); d.className = 'drawer'; var panel = document.createElement('div'); panel.className = 'drawer-panel'; panel.style.width = '640px'; panel.style.maxWidth = '96vw';
    d.appendChild(panel); d.onclick = function (e) { if (e.target === d) d.remove(); }; document.body.appendChild(d);
    function refresh() { panel.innerHTML = body(); wire(); }
    function body() {
      var rows = operations.map(function (o) { var c = opCompany(o); return '<tr><td class="cell-text">' + esc(c ? c.nomeFantasia || c.razaoSocial : '—') + '</td><td class="cell-text">' + esc(o.nome) + '</td><td>' + esc(o.marketplace || '—') + '</td><td class="mono">' + esc(o.storeName || '—') + '</td><td>' + (o.ativa !== false ? '<span class="tag ok">ativa</span>' : '<span class="tag neutral">inativa</span>') + '</td><td><button class="btn-sm" data-optoggle="' + esc(o.id) + '">' + (o.ativa !== false ? 'Desativar' : 'Ativar') + '</button></td></tr>'; }).join('');
      return '<div class="dh"><div><b>Gerenciar operações</b></div><button class="x">&times;</button></div><div class="dbd">' +
        '<div class="table-wrap"><table class="report"><thead><tr><th>Empresa</th><th>Operação</th><th>Marketplace</th><th>Loja</th><th>Status</th><th></th></tr></thead><tbody>' + (rows || '<tr><td colspan="6" class="empty">Nenhuma operação cadastrada.</td></tr>') + '</tbody></table></div>' +
        '<div class="footnote" style="margin-top:8px">Uma operação com dado já importado nunca é excluída — só desativada (some do seletor, mas os dados continuam preservados e acessíveis).</div>' +
        '<div style="margin-top:12px"><button class="btn-sm primary" id="go-nova">+ Nova operação</button></div>' +
        '</div>';
    }
    function wire() {
      panel.querySelector('.x').onclick = function () { d.remove(); };
      panel.querySelector('#go-nova').onclick = function () { d.remove(); openNovaOperacaoModal(); };
      panel.querySelectorAll('[data-optoggle]').forEach(function (b) { b.onclick = function () { var op = operations.find(function (x) { return x.id === b.dataset.optoggle; }); if (op) operationSave(Object.assign({}, op, { ativa: op.ativa === false })).then(refresh); }; });
    }
    refresh();
  }
  ...
  // §12/§54/§156 do prompt "Correção Financeira + Refazer layout": UM ÚNICO FLUXO do dinheiro do dia,
  // em vez de vários blocos técnicos soltos. Não recalcula nada — soma o que os motores canônicos já
  // aprovados (aceleraResultadoFechamento/caixaDayMovimentosPosteriores/caixaDayTransferenciasBancarias)
  // já apuraram. Transferência bancária NUNCA entra em "descontos" (§17/§18 — não é despesa).
  function caixaFluxoDia(dateKey) {
    var ac = aceleraResultadoFechamento(dateKey);
    var movPost = caixaDayMovimentosPosteriores(dateKey);
    var transf = caixaDayTransferenciasBancarias(dateKey);
    // ac.valorLiquido/valorBruto/taxaAcelera vêm em CENTAVOS (convenção do motor do Acelera — mesma
    // usada por brlC() em todo o app); t.amount da Carteira vem em REAIS. Converter aqui para reais
    // antes de somar — nunca misturar as duas unidades na mesma conta (bug real pego em teste: sem
    // essa conversão o "Entrou" aparecia 100× maior).
    var entrouAcelera = ac.resgatesN ? ac.valorLiquido / 100 : 0;
    var outrosCreditos = 0, descontos = 0;
    movPost.todos.forEach(function (t) {
      if (caixaMovCategoria(t, ac) === 'TRANSFERENCIAS_BANCARIAS') return; // saída pro banco — categoria à parte
      if (t.amount > 0) outrosCreditos += t.amount; else descontos += Math.abs(t.amount);
    });
    var entrou = r2(entrouAcelera + outrosCreditos);
    var disponivel = r2(entrou - descontos);
    var transferido = transf.total;
    var diferenca = r2(disponivel - transferido);
    return { entrouAcelera: r2(entrouAcelera), outrosCreditos: r2(outrosCreditos), entrou: entrou, descontos: r2(descontos), disponivel: disponivel, transferido: transferido, diferenca: diferenca, ac: ac, movPost: movPost, transf: transf };
  }
  function openCaixaFechamentoDia(dateKey) {
    ...
    function body() {
      ...
      var vendas = caixaDayVendas(dateKey), exp = caixaDayExpedicao(dateKey), pend = st.pend, pendCart = st.pendCart;
      ...
      var pcat = movPost.porCategoria;
      function caixaCartSituacao(t) {
        if (pendCart.itens.some(function (it) { return it.t.id === t.id; })) return '<span class="tag warn">🔴 Pendência</span>';
        var cc = conc.itens.find(function (it) { return it.t.id === t.id; });
        if (cc) return '<span class="tag ok">🟢 ' + esc(CAIXA_CART_CAT_LABEL[cc.categoria] || 'Conciliado') + '</span>';
        return '<span class="tag neutral">⚪ sem comparação</span>';
      }
      function movRow(t) {
        return '<tr class="rowlink" data-wtx="' + esc(t.id) + '"><td class="nowrap">' + dbr(t.date) + '</td><td class="mono">' + esc(wOrderId(t) || '— não identificado') + '</td><td class="cell-text">' + esc((t.desc || t.tipo || '—').slice(0, 60)) + '</td><td class="nowrap ' + (t.amount < 0 ? 'neg' : 'pos') + '">' + brl(t.amount) + '</td><td>' + caixaCartSituacao(t) + '</td></tr>';
      }

      // ---- Cabeçalho ----
      var opLbl = opLabel(opActive());
      var header = '<div class="dh" style="flex-direction:column;align-items:stretch;gap:2px">' +
        '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;width:100%">' +
        '<div class="cx-head"><b>Fechamento de Caixa</b><span>' + esc(dbr(dateKey)) + '</span><span class="tag ' + lbl[1] + '">' + esc(lbl[0]) + '</span>' + (st.status === 'REVISAO_NECESSARIA' ? ' <span class="footnote" style="margin:0">⚠ movimento posterior a este fechamento</span>' : '') + '</div>' +
        '<div style="display:flex;gap:6px;align-items:center;white-space:nowrap"><button class="btn-sm" id="caixa-xlsx" title="Exportar planilha">Exportar</button><button class="btn-sm" id="caixa-print" title="Imprimir / gerar PDF">Imprimir</button><button class="x">&times;</button></div>' +
        '</div><div class="cx-sub">' + esc(opLbl) + '</div></div>';

      // ---- RESUMO ----
      var diferencaOk = Math.abs(fluxo.diferenca) <= 1;
      var resumo = '<div class="dbd" style="padding-top:14px">' +
        kstrip([
          { l: 'Entrou', v: brl(fluxo.entrou), cls: 'blue' },
          { l: 'Descontos', v: fluxo.descontos ? brl(-fluxo.descontos) : brl(0), cls: fluxo.descontos ? 'red' : '' },
          { l: 'Transferido ao banco', v: brl(fluxo.transferido), cls: 'blue' },
          { l: 'Diferença', v: diferencaOk ? '✓ Conferido' : brl(fluxo.diferenca), cls: diferencaOk ? 'green' : (Math.abs(fluxo.diferenca) > 50 ? 'red' : 'amber') }
        ]);

      // ---- FLUXO DO DINHEIRO ----
      var flowSteps = [
        { l: 'Entrou', v: brl(fluxo.entrou), cls: 'neu' },
        { l: 'Descontos', v: fluxo.descontos ? '−' + brl(fluxo.descontos) : brl(0), cls: fluxo.descontos ? 'neg' : '' },
        { l: 'Disponível', v: brl(fluxo.disponivel), cls: 'neu' },
        { l: 'Banco', v: brl(fluxo.transferido), cls: 'neu' },
      ...
      // os pedidos que ainda não tinham sido mostrados.
      var resgateRows = ac.resgates.map(function (r) { return '<tr class="rowlink" data-goresgate="' + esc(r.resgate) + '"><td class="mono">' + esc(r.resgate) + '</td><td>' + nn(r.n) + '</td><td class="nowrap">' + brlC(r.bruto) + '</td><td class="nowrap neg">' + brlC(-Math.abs(r.taxa)) + '</td><td class="nowrap pos">' + brlC(r.liquido) + '</td></tr>'; }).join('');
      var resgateDetails = ac.resgates.map(function (r) {
        var pedidosTbl = r.pedidos.map(function (p) { var info = exp[p]; var pv = ac.pedidoValores[p]; return '<tr><td class="mono">' + esc(p) + '</td><td class="nowrap">' + (pv ? brlC(pv.bruto) : '—') + '</td><td class="nowrap neg">' + (pv ? brlC(-Math.abs(pv.taxa)) : '—') + '</td><td class="nowrap pos">' + (pv ? brlC(pv.liquido) : '—') + '</td><td>' + (info ? ((info.isFbs ? '🟢 Full confirmado' : '🟢 Bipado') + (info.at ? ' em ' + dbr(info.at) : '')) : '🔴 sem confirmação de expedição') + '</td></tr>'; }).join('');
      ...
      var origem = '<div class="cx-section"><h4>Origem do dinheiro' + h4sub(brl(fluxo.entrou) + ' recebido') + '</h4>' +
        ...
        (fluxo.outrosCreditos ? ('<div class="cx-kv" style="margin-top:10px"><span class="cxl">Outros créditos</span><span class="cxv pos">+' + brl(fluxo.outrosCreditos) + '</span></div>' +
          '<details class="cx-collapse"><summary>Ver créditos</summary><div class="pb"><div class="table-wrap"><table class="report"><thead><tr><th>Data</th><th>Pedido</th><th>Descrição</th><th>Valor</th><th>Situação</th></tr></thead><tbody>' + creditRows + '</tbody></table></div></div></details>') : '') +
        '</div>';

      // ---- DESCONTOS E AJUSTES ----
      var DESC_ORDER = ['DEVOLUCOES_REEMBOLSOS', 'AJUSTES_ACELERA', 'AJUSTES_SHOPEE', 'OUTROS_DEBITOS', 'NAO_IDENTIFICADOS'];
      var descGroups = DESC_ORDER.map(function (k) {
        var g = pcat[k]; if (!g || !g.itens.length) return null;
        var neg = g.itens.filter(function (t) { return t.amount < 0; });
      ...
      var descontosBlock = '<div class="cx-section"><h4>Descontos e ajustes' + h4sub(fluxo.descontos ? brl(-fluxo.descontos) : null) + '</h4>' +
        (descGroups.length ? (descGroups.map(function (g) {
          ...
          : '<div class="footnote">Nenhum desconto identificado neste dia.</div>') +
        '</div>';

      // ---- TRANSFERÊNCIA PARA BANCO — seção ÚNICA (§16-25/§50-52 do prompt "Refazer layout"): nunca
      // mais duplicada dentro de Descontos, nunca soma "automático + manual" exposta como conceito —
      ...
      var dreResumo = '<div class="cx-section"><h4>DRE do dia' + h4sub(dre.n ? (dre.lucro != null ? brlC(dre.lucro) : 'aguardando custo') : null) + '</h4>' +
        (dre.n ? (
          ...
          (dre.custoTotal != null ? ('<div class="cx-kv"><span class="cxl">Custo do produto</span><span class="cxv neg">' + brlC(-dre.custoTotal) + '</span></div>') : '<div class="cx-kv"><span class="cxl">Custo do produto</span><span class="cxv">não disponível</span></div>') +
      ...
      var expRowsC = exp.itens.map(function (i) { return '<tr class="rowlink" data-goped360="' + esc(i.orderId) + '"><td class="mono">' + esc(i.orderId) + '</td><td>' + esc(i.modalidade || '—') + '</td><td class="nowrap">' + (i.expedidoAt ? new Date(i.expedidoAt).toLocaleString('pt-BR') : '—') + '</td><td>' + esc(i.situacao) + '</td></tr>'; }).join('');
      var expedicaoDetails = '<details class="cx-collapse"><summary>Expedição · ' + nn(exp.expedidos) + ' bipado(s)</summary><div class="pb">' + (exp.temSessao ? kv('Esperados / confirmados / faltaram', nn(exp.esperados) + ' / ' + nn(exp.expedidos) + ' / ' + nn(exp.faltaram)) : '') + '<div class="table-wrap"><table class="report"><thead><tr><th>Pedido</th><th>Modalidade</th><th>Confirmado em</th><th>Situação</th></tr></thead><tbody>' + (expRowsC || '<tr><td colspan="4" class="empty">Nenhum pedido expedido/confirmado neste dia.</td></tr>') + '</tbody></table></div></div></details>';
      var pendOperRowsC = pend.operacionais.map(function (i) { return '<tr><td class="mono">' + esc(i.orderId) + '</td><td>' + esc(i.origem) + '</td><td class="cell-text">' + esc(i.motivo) + '</td><td><button class="btn-sm" data-caixapendop="' + esc(i.orderId) + '">Ver</button></td></tr>'; }).join('');
      var pendFinRowsC = pend.financeiras.slice(0, 60).map(function (f) { return '<tr><td class="mono">' + esc(f.orderId) + '</td><td class="cell-text">' + esc(f.motivo) + '</td><td><button class="btn-sm" data-goped360="' + esc(f.orderId) + '">Ver pedido</button> <button class="btn-sm" data-caixapendfin="' + esc(f.orderId) + '">Resolver</button></td></tr>'; }).join('');
      var pendCartRowsC = pendCart.itens.map(function (it) { return '<tr class="rowlink" data-wtx="' + esc(it.t.id) + '"><td class="nowrap">' + dbr(it.t.date) + '</td><td class="mono">' + esc(wOrderId(it.t) || '—') + '</td><td class="cell-text">' + esc(it.motivo) + '</td><td class="nowrap">' + brl(it.t.amount) + '</td></tr>'; }).join('');
      var pendTotal = pend.total + pendCart.total;
      var pendenciasDetails = '<details class="cx-collapse"' + (pendTotal ? ' open' : '') + '><summary>Pendências · ' + (pendTotal ? nn(pendTotal) + ' para resolver' : 'Tudo identificado') + '</summary><div class="pb">' +
        (pend.financeiras.length ? ('<div class="footnote" style="margin:0 0 4px"><b>Pedidos</b></div><div class="table-wrap"><table class="report"><thead><tr><th>Pedido</th><th>Problema</th><th></th></tr></thead><tbody>' + pendFinRowsC + '</tbody></table></div>') : '') +
        (pend.operacionais.length ? ('<div class="footnote" style="margin:10px 0 4px"><b>Expedição / Acelera</b></div><div class="table-wrap"><table class="report"><thead><tr><th>Pedido</th><th>Origem</th><th>Motivo</th><th></th></tr></thead><tbody>' + pendOperRowsC + '</tbody></table></div>') : '') +
        (pendCart.itens.length ? ('<div class="footnote" style="margin:10px 0 4px"><b>Carteira</b></div><div class="table-wrap"><table class="report"><thead><tr><th>Data</th><th>Pedido</th><th>Motivo</th><th>Valor</th></tr></thead><tbody>' + pendCartRowsC + '</tbody></table></div>') : '') +
        (!pendTotal ? '<span class="tag ok">✓ Tudo identificado</span>' : '') +
        '</div></details>';
      var auditoriaDetails = '<details class="cx-collapse"><summary>Auditoria</summary><div class="pb">' +
        kv('Taxas conferidas (Income cruzado)', (vendas.n - vendas.pendFin.length) + ' de ' + vendas.n + ' pedidos') + kv('Custos conferidos', nn(custoConhecido) + ' de ' + nn(vendas.n) + ' pedidos') + kv('Valores não classificados (total importado)', nn(naoClass) + ' coluna(s)') +
        '<div class="footnote" style="margin:10px 0 4px"><b>Saldo da Carteira (verificação técnica)</b></div>' +
        fLine('Saldo inicial da Carteira', saldoCart.saldoInicial != null ? Math.round(saldoCart.saldoInicial * 100) : null) +
        fLine('+ Entradas do dia', Math.round(saldoCart.entradas * 100)) +
        fLine('− Débitos operacionais', -Math.round(saldoCart.debitosOperacionais * 100)) +
        fLine('− Transferências bancárias', -Math.round(saldoCart.transferenciasBancarias * 100)) +
      ...
      var footer = '<div class="cx-footer">' +
        '<div class="footnote" style="margin:0">Diferença: <b>' + brl(fluxo.diferenca) + '</b> · ' + nn(pendTotal) + ' pendência(s)' + (st.rec ? ' · último registro: ' + esc(CAIXA_LABEL[st.rec.status] ? CAIXA_LABEL[st.rec.status][0] : st.rec.status) + ' por ' + esc(st.rec.closedBy || '—') : '') + '</div>' +
      ...
      return header + resumo + flow + origem + descontosBlock + transferBlock + dreResumo + conferencias + '</div>' + footer;
    }
    function wire() {
      panel.querySelector('.x').onclick = function () { d.remove(); };
      panel.querySelectorAll('[data-goped360]').forEach(function (b) { b.onclick = function () { d.remove(); openPedidoFicha360(b.dataset.goped360); }; });
      ...
      // §22-25 do prompt "Refazer layout": lançamento manual é EXCEÇÃO — só aparece atrás de um botão,
      // nunca um formulário permanentemente aberto. Alerta de possível duplicidade antes de registrar
  ...
  // Modal de transferência manual — só aberto sob demanda (nunca formulário permanente no fechamento).
  function openTransferenciaManualModal(dateKey, onSaved) {
    var transfAtual = caixaDayTransferenciasBancarias(dateKey);
    var o = document.createElement('div'); o.className = 'overlay'; o.style.zIndex = '70'; o.innerHTML = '<div class="modal" style="width:420px">' +
      '<div class="mh"><h3>Adicionar transferência não encontrada</h3><button class="x">×</button></div><div class="mbd">' +
      '<label class="fld">Conta destino</label><select class="select" id="bt-conta" style="width:100%">' + ('<option value="">Selecione a conta…</option>' + bankAccountsForActiveCompany().filter(function (b) { return b.ativa !== false; }).map(function (b) { return '<option value="' + esc(b.id) + '">' + esc(b.nome) + (b.banco ? ' — ' + esc(b.banco) : '') + '</option>'; }).join('') + '<option value="__novo__">+ Cadastrar nova conta</option>') + '</select>' +
      '<label class="fld">Valor (R$)</label><input class="input" id="bt-valor" style="width:100%" placeholder="0,00">' +
      '<label class="fld">Data/hora</label><input class="input" type="datetime-local" id="bt-quando" value="' + esc(dateKey) + 'T12:00" style="width:100%">' +
      '<label class="fld">Observação (opcional)</label><input class="input" id="bt-obs" style="width:100%">' +
      '<div id="bt-err"></div></div><div class="mf"><button class="btn-sm" id="bt-cancel">Cancelar</button><button class="btn-sm primary" id="bt-registrar">Registrar transferência</button></div></div>';
    o.onclick = function (e) { if (e.target === o) o.remove(); };
    document.body.appendChild(o);
    o.querySelector('.x').onclick = o.querySelector('#bt-cancel').onclick = function () { o.remove(); };
    var contaSel = o.querySelector('#bt-conta'); contaSel.onchange = function () {
      if (contaSel.value !== '__novo__') return;
      contaSel.value = '';
      openCadastrarContaModal(function (full) { var opt = document.createElement('option'); opt.value = full.id; opt.textContent = full.nome + (full.banco ? ' — ' + full.banco : ''); contaSel.insertBefore(opt, contaSel.lastElementChild); contaSel.value = full.id; });
    };
    o.querySelector('#bt-registrar').onclick = function () {
      var valor = walletNum(o.querySelector('#bt-valor').value);
      var errEl = o.querySelector('#bt-err');
      if (valor == null || valor <= 0) { errEl.innerHTML = '<div class="form-err">Informe um valor válido.</div>'; return; }
      var quando = o.querySelector('#bt-quando').value; if (!quando) { errEl.innerHTML = '<div class="form-err">Informe a data/hora.</div>'; return; }
      // §25 do prompt "Refazer layout": nunca deixar duplicar silenciosamente um Pix já importado.
      var possivelDup = transfAtual.autoTxs.some(function (a) { return Math.abs(Math.abs(a.amount) - valor) <= 1; });
      if (possivelDup && !confirm('Já existe uma transferência semelhante importada da Carteira. Deseja realmente registrar outra?')) return;
      var contaId = contaSel.value || null; if (contaId === '__novo__') contaId = null; var conta = contaId ? bankAccountLabel(contaId) : null; var obs = o.querySelector('#bt-obs').value.trim() || null;
      caixaRegistrarTransferenciaBanco(valor, new Date(quando).toISOString(), conta, obs, contaId).then(function () { o.remove(); toast('Transferência registrada', brl(valor)); if (onSaved) onSaved(); render(); });
    };
  }
  ...
  function caixaExportarXlsx(dateKey) {
    var vendas = caixaDayVendas(dateKey), exp = caixaDayExpedicao(dateKey), ac = aceleraResultadoFechamento(dateKey), pend = caixaDayPendencias(dateKey), pendCart = caixaDayPendenciasCarteira(dateKey), mov = caixaDayMovimentosCarteira(dateKey), conc = caixaDayLancamentosConciliados(dateKey);
    var transf = caixaDayTransferenciasBancarias(dateKey);
    var saldoCart = caixaDaySaldoCarteira(dateKey);
    ...
    var resumo = [['Fechamento de Caixa', dbr(dateKey)], [],
      ...
      ['Recebimentos de pendências anteriores (qtd)', ac.pedidosAntigosRecebidosHoje.length], ['Recebimentos de pendências anteriores (R$)', ac.valorAntigosRecebidosHoje / 100],
      ['Outras receitas conciliadas (R$)', outrasReceitasValor / 100],
      ['Transferido para o banco — automático (R$)', transf.autoTotal], ['Transferido para o banco — manual (R$)', transf.manualTotal], ['Transferido para o banco — total (R$)', transf.total],
      ['Saldo inicial da Carteira (R$)', saldoCart.saldoInicial != null ? saldoCart.saldoInicial : ''], ['Saldo final calculado da Carteira (R$)', saldoCart.saldoFinalCalculado != null ? saldoCart.saldoFinalCalculado : ''], ['Saldo final informado (R$)', saldoCart.saldoFinalInformado != null ? saldoCart.saldoFinalInformado : ''], ['Diferença não explicada (R$)', saldoCart.diferenca != null ? saldoCart.diferenca : ''],
      ['Créditos na Carteira (qtd)', mov.creditos.length], ['Débitos na Carteira (qtd)', mov.debitos.length],
      ['Pendência não explicada — Carteira (R$)', pendNaoExplicadaValor],
      ['Pendências do fluxo', pend.total], ['Pendências da carteira', pendCart.total]];
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(resumo), 'Resumo');
    var conciliacaoRows = [['Item', 'Valor (R$)'],
      ['Saldo inicial da Carteira', saldoCart.saldoInicial != null ? saldoCart.saldoInicial : ''],
      ['+ Entradas do dia', saldoCart.entradas], ['- Débitos operacionais', -saldoCart.debitosOperacionais], ['- Transferências bancárias', -saldoCart.transferenciasBancarias],
      ['= Saldo final calculado', saldoCart.saldoFinalCalculado != null ? saldoCart.saldoFinalCalculado : ''],
      ['Saldo final informado (Carteira)', saldoCart.saldoFinalInformado != null ? saldoCart.saldoFinalInformado : ''],
      ['Diferença', saldoCart.diferenca != null ? saldoCart.diferenca : '']];
    ...
    var acRows2 = [['Resgate', 'Pedidos', 'Bruto (R$)', 'Taxa (R$)', 'Líquido (R$)', 'Divergente']];
    ac.resgates.forEach(function (r) { acRows2.push([r.resgate, r.n, r.bruto / 100, r.taxa / 100, r.liquido / 100, r.divergente ? 'sim' : 'não']); });
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(acRows2), 'Acelera');
    var receitaRows2 = [['Tipo', 'Pedido', 'Descrição', 'Valor (R$)', 'Origem']];
    ['PENDENCIA_ANTERIOR', 'COMPENSACAO', 'OUTRAS_RECEITAS'].forEach(function (k) { var g = pc[k]; if (!g) return; g.itens.forEach(function (it) { receitaRows2.push([CAIXA_CART_CAT_LABEL[k], wOrderId(it.t) || '—', it.t.desc || it.t.tipo || '—', it.t.amount, it.origem]); }); });
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(receitaRows2), 'Receitas Adicionais');
    ...
    var transfRows2 = [['Origem', 'Data/hora', 'Descrição/Conta', 'Valor (R$)']];
    transf.autoTxs.forEach(function (t) { transfRows2.push(['Automático (Carteira)', dbr(t.date), t.desc || t.tipo || '—', Math.abs(t.amount)]); });
    transf.manual.forEach(function (t) { transfRows2.push(['Manual', new Date(t.quando).toLocaleString('pt-BR'), t.conta || '—', t.valor]); });
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(transfRows2), 'Transferências Bancárias');
  ...
  function caixaImprimirFechamento(dateKey) {
    ...
    var html = '<div class="pdoc">' +
      ...
      '<div class="psection"><h2>Conferência Expedição × Acelera</h2>' + prow('Expedidos esperados', ac.expedidosEsperadosN) + prow('Encontrados no Acelera', ac.expedidosEncontradosN) + prow('Não localizados', ac.expedidosNaoEncontrados.length) + prow('Acelera sem confirmação interna', ac.aceleraSemConfirmacaoExpedicao.length) + '</div>' +
      '<div class="psection"><h2>Conciliação do Dinheiro</h2>' + prow('Saldo inicial da Carteira', saldoCart.saldoInicial != null ? saldoCart.saldoInicial : 'sem saldo inicial conhecido') + prow('+ Entradas do dia', saldoCart.entradas) + prow('- Débitos operacionais', -saldoCart.debitosOperacionais) + prow('- Transferências bancárias', -saldoCart.transferenciasBancarias) + prow('= Saldo final calculado', saldoCart.saldoFinalCalculado != null ? saldoCart.saldoFinalCalculado : '—', true) + (saldoCart.saldoFinalInformado != null ? prow('Saldo final informado (Carteira)', saldoCart.saldoFinalInformado) + prow('Diferença', saldoCart.diferenca) : '') + '</div>' +
      '<div class="psection"><h2>Receitas Adicionais</h2>' + prow('Recebimentos de pendências anteriores', ac.valorAntigosRecebidosHoje / 100) + prow('Compensações / Recuperações', (pc.COMPENSACAO ? pc.COMPENSACAO.valor : 0)) + prow('Outras receitas classificadas', (pc.OUTRAS_RECEITAS ? pc.OUTRAS_RECEITAS.valor : 0)) + prow('Total de Outras Receitas', outrasReceitasValor, true) + '</div>' +
      ...
      '<div class="psection"><h2>Transferências Bancárias</h2>' + thTable(['Origem', 'Data/hora', 'Descrição/Conta', 'Valor'], transf.autoTxs.map(function (t) { return '<tr><td>Automático (Carteira)</td><td>' + dbr(t.date) + '</td><td>' + esc(t.desc || t.tipo || '—') + '</td><td>' + brl(Math.abs(t.amount)) + '</td></tr>'; }).concat(transf.manual.map(function (t) { return '<tr><td>Manual</td><td>' + new Date(t.quando).toLocaleString('pt-BR') + '</td><td>' + esc(t.conta || '—') + '</td><td>' + brl(t.valor) + '</td></tr>'; }))) + prow('Total transferido', transf.total, true) + '</div>' +
  ...
  openDB().catch(function (e) { activateMemoryMode(e && (e.message || '') || 'IndexedDB indisponível'); }).then(function () {
    ...
    // §BK/§BL do complemento — MIGRAÇÃO MULTI-OPERAÇÃO: idempotente (só roda se ainda não existe
    ...
    return opRunMigrationIfNeeded({ orders: orders, occ: occ, wallet: wallet, acelera: acelera, affConv: affConv, affRpa: affRpa, affVb: affVb, mrRenda: mrRenda, mrShip: mrShip, mrAdj: mrAdj, mrSvc: mrSvc }).then(function () {
      ...
      // uma única vez, nunca espalhada por centenas de funções. "Todas as Operações" não filtra
      // (visão consolidada — nunca mistura o dado GRAVADO, só agrega a leitura).
      if (activeOperationId !== OP_ALL) {
        var byOp = function (arr) { return arr.filter(function (x) { return !x.operationId || x.operationId === activeOperationId; }); };
