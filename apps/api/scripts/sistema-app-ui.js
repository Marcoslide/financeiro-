...
(function () {
  ...
  function importPosVenda(type, file) {
    return file.arrayBuffer().then(function (ab) {
      ...
      Object.keys(groups).forEach(function (key) {
        ...
        var next = { id: uid, type: type, orderId: rep.orderId, returnId: rep.returnId, status: rep.status, reason: rep.reason, resolution: rep.resolution, occurredAt: rep.occurredAt ? new Date(rep.occurredAt).toISOString() : null, requested: requested, compensation: compensation, exposure: S.posVenda.classify(rep.status, requested, compensation), items: g.map(function (r) { return { sku: r.sku, productName: r.productName, qty: r.quantity }; }) };
        var ex = byId[uid]; if (!ex) { novo++; } else if (ex.status !== next.status) { upd++; } else { unch++; } byId[uid] = next; changed.push(next);
      });
  ...
  // ============================================================ RENDER (roteamento)
  function setActive() { document.querySelectorAll('#nav a').forEach(function (a) { a.classList.toggle('active', a.dataset.route === route); }); crumb.textContent = { dashboard: 'Dashboard', produtos: 'Produtos', pedidos: 'Pedidos', posvenda: 'Pós-venda', ia: 'Inteligência' }[route]; }
  function render() {
    setActive();
  ...
  function renderPosVenda() {
    ...
    app.innerHTML =
      ...
      (sub.posvenda === 'import' ? '<div class="cards6">' + TYPES.map(function (t) { return '<div class="fcard"><div class="lbl">' + t[1] + '</div><button class="btn-sm primary" style="margin-top:10px" data-pv="' + t[0] + '">Importar</button></div>'; }).join('') + '</div>' + importsFor('Pós-venda') : sub.posvenda === 'ocorrencias' ? pvOccList() : pvVisao());
  ...
  function pvVisao() {
    var list = occInPeriod(); if (!list.length) return emptyBox('Nenhuma ocorrência. Importe os 3 relatórios de pós-venda.');
    var e = sumExposure(list); var byType = {}; list.forEach(function (o) { byType[o.type] = (byType[o.type] || 0) + 1; });
    ...
    return '<div class="cards6">' + fcard('Ocorrências', nn(list.length), 'blue') + fcard('Devoluções', nn(byType.RETURN_REFUND || 0), '') + fcard('Cancelamentos', nn(byType.ORDER_CANCELLATION || 0), '') + fcard('Falhas de entrega', nn(byType.FAILED_DELIVERY || 0), '') + fcard('Prejuízo confirmado', brl(e.confirmedLoss), 'red') + fcard('Em risco', brl(e.atRisk), 'amber') + fcard('Recuperado', brl(e.recovered), 'green') + fcard('Solicitação cancelada', brl(e.cancelled), '') + '</div>' +
      '<div class="panel"><div class="ph"><h3>Top SKUs por ocorrência</h3></div><div class="table-wrap"><table><thead><tr><th>SKU</th><th>Produto</th><th>Ocorrências</th></tr></thead><tbody>' + top.map(function (m) { return '<tr><td class="mono">' + esc(m.sku) + '</td><td>' + esc((m.product || '').slice(0, 50)) + '</td><td>' + nn(m.occ) + '</td></tr>'; }).join('') + '</tbody></table></div></div><div class="footnote">' + esc(S.posVenda.methodology) + '</div>';
  }
  function pvOccList() {
    var list = occInPeriod(); if (!list.length) return emptyBox('Nenhuma ocorrência importada.');
    var ordSet = {}; orders.forEach(function (o) { ordSet[o.id] = true; });
    return '<div class="count-line"><b>' + nn(list.length) + '</b> ocorrências</div><div class="panel"><div class="table-wrap"><table><thead><tr><th>Tipo</th><th>Pedido</th><th>Status</th><th>Itens</th><th>Solicitado</th><th>Exposição</th><th>Pedido?</th><th></th></tr></thead><tbody>' + list.slice(0, 300).map(function (o) { return '<tr><td><span class="tag info">' + esc(o.type.split('_')[0]) + '</span></td><td class="mono">' + esc(o.orderId || '—') + '</td><td>' + esc((o.status || '—')) + '</td><td>' + (o.items || []).length + '</td><td>' + brl(o.requested) + '</td><td><span class="tag ' + (o.exposure.bucket === 'CONFIRMED' ? 'warn' : o.exposure.bucket === 'AT_RISK' ? 'info' : 'ok') + '">' + o.exposure.bucket + '</span></td><td>' + (ordSet[o.orderId] ? '<span class="tag ok">vinculado</span>' : '—') + '</td><td><button class="btn-sm" data-oc="' + esc(o.id) + '">Abrir</button></td></tr>'; }).join('') + '</tbody></table></div></div>';
  }
  function openOcc(id) {
    var o = occ.find(function (x) { return x.id === id; }); if (!o) return; var ord = orders.find(function (x) { return x.id === o.orderId; });
    var d = document.createElement('div'); d.className = 'drawer';
    d.innerHTML = '<div class="drawer-panel"><div class="dh"><b>Ficha da ocorrência</b><button class="x">&times;</button></div><div class="dbd">' + kv('Tipo', o.type) + kv('Pedido', o.orderId) + kv('Status', o.status) + kv('Reembolso solicitado', brl(o.requested)) + kv('Exposição', o.exposure.bucket) + kv('Motivo', o.reason) +
      ...
      '<div class="panel" style="margin-top:12px"><div class="ph"><h3>Itens (valor contado uma vez)</h3></div><div class="pb">' + (o.items || []).map(function (i) { return '<div class="ro" style="margin-bottom:6px"><span class="mono">' + esc(i.sku || '—') + '</span> · ' + esc((i.productName || '—')) + '</div>'; }).join('') + '</div></div></div></div>';
    d.onclick = function (e) { if (e.target === d) d.remove(); }; d.querySelector('.x').onclick = function () { d.remove(); }; document.body.appendChild(d);
  }
  ...
  function makeProdutos(opts) {
    ...
    function renderTable(masters, pageAll) {
      ...
      var body = masters.map(function (m) {
        ...
        var master = '<tr class="master-row" data-pid="' + p.id + '"><td><input type="checkbox" class="chk chk-m" data-pid="' + p.id + '"' + (allSel ? ' checked' : '') + ' data-indet="' + (indet ? 1 : 0) + '"></td><td><button class="expander" data-exp="' + p.id + '">' + (open ? '▾' : '▸') + '</button></td><td><div class="pname">' + esc(p.name) + (p.status === 'INACTIVE' ? ' <span class="tag">inativo</span>' : '') + '</div><div class="footnote" style="margin:0">' + (p.principalSku ? 'SKU: <span class="mono">' + esc(p.principalSku) + '</span> · ' : '') + 'ID Shopee: <span class="mono">' + esc(p.shopeeProductId) + '</span></div></td><td>' + shown.length + (shown.length < m.all.length ? ' <span class="footnote" style="margin:0">de ' + m.all.length + '</span>' : '') + '</td><td class="cell-fam" data-mfam="' + p.id + '">' + famCell(a.famSummary) + '</td><td>' + range + '</td><td class="cell-close" data-mclose="' + p.id + '">' + money1(a.close) + '</td><td>' + cost1(a.cost) + '</td><td>' + nn(a.totalStock) + '</td></tr>';
        var subs = '';
        if (open) { subs = '<tr class="subwrap"><td colspan="9" style="padding:0"><table><thead><tr><th style="width:30px"></th><th>Variação</th><th>Família</th><th>Preço Shopee</th><th>Preço Fechamento</th><th>Custo (herdado)</th><th>Estoque</th></tr></thead><tbody>' + shown.map(function (v) { var f = v.familyId ? famById()[v.familyId] : null; return '<tr class="subrow" data-vid="' + v.id + '"><td><input type="checkbox" class="chk chk-v" data-vid="' + v.id + '"' + (S2.selected.has(v.id) ? ' checked' : '') + '></td><td class="vname">' + esc(v.variationName || '(única)') + '<div class="footnote" style="margin:0">SKU: <span class="mono">' + esc(v.sku || '—') + '</span></div></td><td class="cell-fam" data-vfam="' + v.id + '">' + (f ? '<span class="tag info">' + esc(f.name) + '</span>' : '<span class="tag warn">sem família</span>') + '</td><td>' + brl(v.shopeeFullPrice) + '</td><td class="cell-close" data-vclose="' + v.id + '">' + (v.closingPrice != null ? brl(v.closingPrice) : '<span class="tag warn">não informado</span>') + '</td><td>' + (f && f.currentCostAmount != null ? brl(f.currentCostAmount) + ' <span class="inh-cost">herdado</span>' : '—') + '</td><td>' + (v.sellerStock == null ? '—' : v.sellerStock) + '</td></tr>'; }).join('') + '</tbody></table></td></tr>'; }
        return master + subs;
      }).join('');
    ...
    function editVarFamily(cell, vid) { var v = S2.variations.find(function (x) { return x.id === vid; }); if (!v) return; var actives = S2.families.filter(function (f) { return f.status === 'ACTIVE' || f.id === v.familyId; }); cell.innerHTML = '<select class="select sm inl"><option value="">— sem família —</option>' + actives.map(function (f) { return '<option value="' + f.id + '"' + (v.familyId === f.id ? ' selected' : '') + '>' + esc(f.name) + '</option>'; }).join('') + '</select>'; var se = cell.querySelector('select'); se.focus(); se.onchange = function () { v.familyId = se.value || null; saveVars([v]).then(function () { refresh(); toast('Salvo', 'Família da variação atualizada.'); }); }; se.onkeydown = function (e) { if (e.key === 'Escape') refresh(); }; se.onblur = function () { setTimeout(function () { if (document.body.contains(se)) refresh(); }, 150); }; }
    ...
    function editMasterFamily(cell, pid, masters) { var m = masters.find(function (x) { return x.p.id === pid; }); if (!m) return; var actives = S2.families.filter(function (f) { return f.status === 'ACTIVE'; }); cell.innerHTML = '<select class="select sm inl"><option value="">— sem família —</option>' + actives.map(function (f) { return '<option value="' + f.id + '">' + esc(f.name) + '</option>'; }).join('') + '</select>'; var se = cell.querySelector('select'); se.focus(); se.onkeydown = function (e) { if (e.key === 'Escape') refresh(); }; se.onchange = function () { var fid = se.value || null; var shown = m.shown, mixed = uniq(shown.map(function (v) { return v.familyId || null; })); var needConfirm = shown.length > 1 && (mixed.length > 1 || (mixed.length === 1 && mixed[0] !== fid && mixed[0] != null)); var apply = function () { shown.forEach(function (v) { v.familyId = fid; }); saveVars(shown).then(function () { refresh(); toast('Família aplicada', shown.length + ' SKUs classificados' + (fid ? ' em “' + famName(fid) + '”' : '') + '.'); }); }; if (needConfirm) confirmModal('Aplicar família a ' + shown.length + ' variações?', 'Este anúncio possui variações com classificações diferentes. Aplicar “' + (fid ? famName(fid) : 'sem família') + '” substituirá as famílias atuais.', apply, refresh); else apply(); }; }
