...
(function () {
  ...
  var pedDashBasis = 'criacao'; // 'criacao' | 'pagamento'
  ...
  // §5-7: "Retornos e Pedidos Cancelados" não pode depender só do texto "Cancelado" — usa também
  // Status da Devolução/Reembolso (Solicitação aprovada, Devolução em Andamento/Concluída) e o
  // motivo de cancelamento, sem sobrescrever o status original da Shopee (§6).
  function pedidoIsRetornoCancelado(o) { return o.normalizedStatus === 'CANCELADO' || !!(o.returnRefundStatus && o.returnRefundStatus.trim()) || !!(o.cancelReason && o.cancelReason.trim()); }
  function pedidosList() {
    ...
    // (agrupado na importação), nunca por linha/item da planilha.
    var counts = { ALL: all.length };
    ['NAO_PAGO', 'A_ENVIAR', 'ENVIADO', 'CONCLUIDO'].forEach(function (k) { counts[k] = all.filter(function (o) { return o.normalizedStatus === k; }).length; });
    counts.CANCELADO = all.filter(pedidoIsRetornoCancelado).length;
    var TAB_DEFS = S.pedidos.tabs.map(function (t) { return t.key === 'CANCELADO' ? { key: 'CANCELADO', label: 'Retornos e Pedidos Cancelados' } : t; });
    ...
    var list = pedTab === 'ALL' ? all : pedTab === 'CANCELADO' ? all.filter(pedidoIsRetornoCancelado) : all.filter(function (o) { return o.normalizedStatus === pedTab; });
    ...
    return '<div class="tabs">' + tabs + '</div><div class="toolbar2"><input class="input sm" id="ped-q" style="width:280px" placeholder="Buscar ID, BR, SKU, produto…" value="' + esc(q) + '"></div>' +
  ...
  function renderAcelera() {
    // §34-35 do prompt "Alterações — Sistema Marketplace Líder": Coortes e Plano de Ação saem da
    // navegação (junto com Capital/Simulador, já removidos antes) — módulo fica focado em
    // Visão Geral, Antecipações/Conferência (Pedidos Expedidos × Acelera), Divergências
    // (Devoluções & Desperdício / Alíquotas), Auditoria e Configurações/Importações.
    var tabs = [['visao', 'Visão Geral'], ['antecipacoes', 'Antecipações'], ['bipados', 'Pedidos Expedidos × Acelera'], ['desperdicio', 'Devoluções & Desperdício'], ['aliquotas', 'Alíquotas'], ['auditoria', 'Auditoria'], ['config', 'Configurações']];
    ...
    try {
      ...
      else body.innerHTML = ({ visao: aceleraVisao, antecipacoes: aceleraAntecipacoes, bipados: aceleraBipados, desperdicio: aceleraDesperdicio, aliquotas: aceleraAliquotas, auditoria: aceleraAuditoria, config: aceleraConfig }[aceleraSub] || aceleraVisao)();
  ...
  function aceleraConfig() {
    ...
    return head + '<div class="panel"><div class="pb"><div style="display:flex;gap:14px;flex-wrap:wrap">' +
      ...
      '<div class="footnote" style="margin-top:8px">As configurações persistem no navegador e alimentam a Visão Geral (capital para independência, alíquota) e as Antecipações.</div></div></div>';
  }
  ...
  function mrParseIncome(ab, filename) {
    ...
    // Renda
    var rd = mrAoa(wb, 'Renda'); if (rd) { var hi = rd.findIndex(function (r) { return normStatus(r[2]) === 'id do pedido'; }); if (hi >= 0) { for (var i = hi + 1; i < rd.length; i++) { var r = rd[i]; var oid = String(r[2] || '').trim(); if (!oid) continue; var ver = /sku/i.test(String(r[1])) ? 'Sku' : 'Order'; out.renda.push({ ver: ver, orderId: oid, refundId: String(r[3] || '').trim(), sku: String(r[4] || '').trim(), produto: String(r[5] || '').trim(), dataCriacao: String(r[6] || '').trim(), dataConclusao: String(r[7] || '').trim(), canal: String(r[8] || '').trim(), tipoPedido: String(r[9] || '').trim(), liberado: mrCents(r[11]), preco: mrCents(r[12]), reembolso: mrCents(r[13]), pix: mrCents(r[14]), cupom: mrCents(r[15]), fretePagoComprador: mrCents(r[16]), freteParceiro: mrCents(r[17]), descontoFrete: mrCents(r[18]), envioReverso: mrCents(r[19]), comissao: mrCents(r[27]), servico: mrCents(r[28]), transacao: mrCents(r[29]), afiliado: mrCents(r[30]), comissaoBruta: mrCents(r[43]), servicoBruta: mrCents(r[44]) }); } } }
  ...
  function openMrDrill(kind, label) {
    ...
    if (kind === 'taxasShopee') {
      ...
      orders.filter(function (o) { return inPeriod(o.createdAt); }).forEach(function (o) { var f = orderFinance(o); if (f.costPending) return; var cC = Math.round((f.productCostTotal || 0) * 100); if (!cC) return; rows.push({ orderId: o.id, data: o.createdAt, sku: (o.items[0] && o.items[0].sku) || '—', produto: (o.items[0] && o.items[0].productName) || '—', descricao: 'Custo do produto', valor: -cC }); });
    } else if (kind === 'faturamento' || kind === 'lucro') {
      var profitOf = mrOrderProfitEngine(); var mrByOrder = {}; e.orders.forEach(function (r) { mrByOrder[r.orderId] = r; });
      orders.filter(function (o) { return inPeriod(o.createdAt); }).forEach(function (o) {
        var p = profitOf(o); if (!p.known) return;
        var mrRow = mrByOrder[o.id]; var sk = mrRow ? e.skuByOrder[o.id] : null;
        var valor = kind === 'lucro' ? p.lucro : p.receita;
        rows.push({ orderId: o.id, data: o.createdAt, sku: sk && sk[0] ? sk[0].sku : ((o.items[0] && o.items[0].sku) || '—'), produto: sk && sk[0] ? sk[0].produto : ((o.items[0] && o.items[0].productName) || '—'), descricao: mrRow ? 'Minha Renda' : 'estimado (Pedidos)', valor: valor });
      });
    }
  ...
  function mrWaterfall(t) {
    ...
    var steps = [['Preço do produto', t.preco, 'base'], ['Reembolso', t.reembolso, 'neg'], ['PIX', t.pix, 'neg'], ['Cupom', t.cupom, 'neg'], ['Comissão', t.comissao, 'neg'], ['Serviço', t.servico, 'neg'], ['Afiliados', t.afiliado, 'neg'], ['Frete parceiro', t.freteParceiro, 'neg'], ['Desconto frete (Shopee)', t.descontoFrete, 'pos'], ['Envio reverso', t.envioReverso, 'neg']];
  ...
  function mrVisao() {
    ...
    var strip1 = kstrip([
      { l: 'Faturamento Bruto', v: temLucro ? brlC(pe.faturamento) : 'não disponível', cls: 'blue', s: nn(pe.n) + ' pedidos no período', drill: temLucro ? 'faturamento' : null, drillLabel: 'Faturamento Bruto' },
      { l: 'Receita Líquida', v: brlC(t.liberado), cls: 'blue', s: nn(t.nMR) + ' de ' + nn(pe.n) + ' com dados da Shopee', drill: t.nMR ? 'receitaLiquida' : null, drillLabel: 'Receita Líquida' },
      { l: 'Lucro', v: temLucro ? brlC(pe.lucro) : 'não disponível', cls: !temLucro ? 'blue' : (pe.lucro >= 0 ? 'green' : 'red'), drill: temLucro ? 'lucro' : null, drillLabel: 'Lucro' },
    ...
    var strip2 = kstrip([
      ...
      { l: 'Custo dos Produtos', v: custoProdutos != null ? brlC(custoProdutos) : 'não disponível', cls: 'amber', drill: custoProdutos != null ? 'custoProdutos' : null, drillLabel: 'Custo dos Produtos' },
    ]);
    var strip3 = kstrip([
    ...
    return head + strip1 + strip2 + strip3 + coverage + mrWaterfall(t) + tendLink + dedup + shipBox + mrAlertas();
  }
  ...
  function mrTendencias() {
    ...
    var head = secHead('MINHA RENDA · TENDÊNCIAS', 'Como o período atual se compara ao anterior', 'Mesma duração do período selecionado no topo da tela, deslocada para trás — não é necessariamente o "mês anterior" no calendário.');
    var ticketMedio = temLucro ? Math.round(pe.faturamento / pe.lucroN) : null;
    var lucroMedioPedido = temLucro ? Math.round(pe.lucro / pe.lucroN) : null;
    ...
    var chart = chartCard('Faturamento e Lucro — atual × anterior', legendSwatch([['Anterior', '#94a3b8'], ['Atual', '#2b4bd6']]), svgGroupBars(['Faturamento', 'Lucro'], [{ name: 'Anterior', color: '#94a3b8', vals: [(pvFat || 0) / 100, (pvLucro || 0) / 100] }, { name: 'Atual', color: '#2b4bd6', vals: [(temLucro ? pe.faturamento : 0) / 100, (temLucro ? pe.lucro : 0) / 100] }], { fmt: function (v) { return brl(v); } }));
  ...
  function mrDRE() {
    ...
    var cascata = '<div class="panel"><div class="ph"><h3>Cascata</h3><span class="footnote" style="margin:0">clique numa linha para ver os lançamentos</span></div><div class="pb">' +
      line('Receita Bruta', receitaBruta, { note: nn(t.nMR) + ' pedidos com dados da Shopee no período', drill: 'preco' }) +
      line('Descontos Comerciais', descComerciais, { op: '−', note: 'cupom + PIX' }) +
      line('Taxas Shopee', taxasShopee, { op: '−', note: 'comissão, serviço, transação, frete parceiro, ajuste de frete, envio reverso e afiliados — detalhamento na aba Taxas Shopee', drill: 'taxasShopee' }) +
      line('Devoluções e Reembolsos', devolucoes, { op: '−', drill: 'reembolso' }) +
      ...
      line('Receita Líquida', receitaLiquida, { total: true, op: '=', drill: 'receitaLiquida' }) +
      line('Custo dos Produtos', custoProdutos, { op: '−', note: custoNote, drill: 'custoProdutos' }) +
      line('Custos Fixos/Internos', null, { op: '−', note: 'sem fonte de dados no sistema' }) +
      line('Lucro', lucro, { total: true, op: '=', drill: 'lucro' }) +
  ...
  function mrCategorias() {
    ...
    var taxas = grp('TAXAS SHOPEE', [['Comissão', t.comissao, 'comissao'], ['Serviço', t.servico, 'servico'], ['Transação', t.transacao, 'transacao'], ['Programa de frete (frete parceiro)', t.freteParceiro, 'freteParceiro'], ['Ajuste de frete (quando custo)', t.descontoFrete < 0 ? t.descontoFrete : 0, 'descontoFrete']]);
    ...
    var custos = pe.custoProdN > 0 ? grp('CUSTOS INTERNOS', [['Custo dos produtos', pe.custoProd, 'custoProdutos']]) : callout('warn', 'CUSTOS INTERNOS — Custo dos produtos não disponível', 'Nenhum pedido do período tem custo cadastrado em Produtos — não é R$ 0, é "sem dado".');
  ...
  function mrAuditoria() {
    var head = secHead('MINHA RENDA · AUDITORIA', 'Proveniência e importações', 'Cada número tem origem rastreável: arquivo, aba, tipo, pedido.');
    ...
    var e = mrEngine();
    var counts = kstrip([{ l: 'Linhas Order', v: nn(e.orders.length), cls: 'blue' }, { l: 'Linhas SKU', v: nn(mrRenda.length - e.orders.length), cls: 'blue' }, { l: 'Divergências de frete', v: nn(mrShip.length), cls: 'amber' }, { l: 'Ajustes', v: nn(mrAdj.length), cls: 'amber' }, { l: 'Service Fee (linhas)', v: nn(mrSvc.length), cls: 'blue' }, { l: 'Declarações PDF', v: nn(mrPdf.length), cls: 'green' }]);
    return head + counts + '<div class="panel"><div class="ph"><h3>Importações</h3></div><div class="table-wrap"><table class="report"><thead><tr><th>Quando</th><th>Arquivo</th><th>Linhas</th><th>Novos</th></tr></thead><tbody>' + (impRows || '<tr><td colspan="4" class="empty">—</td></tr>') + '</tbody></table></div></div>';
  }
