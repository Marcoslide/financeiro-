...
(function () {
  ...
  var bankTransfers = []; // { id, valor(R$), quando(ISO), conta, observacao, criadoPor, createdAt }
  ...
  var pedExpView = 'fila'; // 'fila' (bipagem) | 'prazos' (Tempo de Envio, absorvido em Expedição)
  ...
  function pedidosExpedicao() {
    ...
    var aEnviarAll = orders.filter(function (o) { return o.normalizedStatus === 'A_ENVIAR'; });
    var fullCount = aEnviarAll.filter(function (o) { return o.isFbs; }).length;
    var baseList = pedExpF.includeFull ? aEnviarAll : aEnviarAll.filter(function (o) { return !o.isFbs; });
    ...
    // anomalia (§4): bipado mas o pedido não está (mais) em A Enviar — status Shopee já avançou,
    // foi cancelado, ou o ID bipado nem existe em Pedidos. Nunca escondido — sempre mostrado à parte.
    ...
    var strip = kstrip([
      { l: 'A Enviar', v: nn(baseList.length), cls: 'blue' },
      { l: 'Bipados', v: nn(bipados.length), cls: 'green' },
      { l: 'Faltando', v: nn(faltando.length), cls: faltando.length ? 'amber' : 'green' },
      { l: 'Bipados fora da fila A Enviar', v: nn(bipadosForaFila.length), cls: bipadosForaFila.length ? 'red' : 'green', s: 'já não está mais em A Enviar' },
    ]);
    ...
    var list = baseList.filter(function (o) { if (pedExpF.status === 'expedido' && !pedIsExpedido(o.id)) return false; if (pedExpF.status === 'pendente' && pedIsExpedido(o.id)) return false; if (q && o.id.toLowerCase().indexOf(q) < 0 && (o.tracking || '').toLowerCase().indexOf(q) < 0) return false; return true; }).sort(function (a, b) { var ba = shipBip[a.id], bb = shipBip[b.id]; if (ba && bb) return bb.bipedAt.localeCompare(ba.bipedAt); if (ba) return -1; if (bb) return 1; return (b.createdAt || '').localeCompare(a.createdAt || ''); });
    var chips = [['', 'Todos'], ['expedido', 'Bipados'], ['pendente', 'Faltando']];
    var rows = list.slice(0, 200).map(function (o) { var b = shipBip[o.id]; var st = b ? pedidoConciliacaoStatus(o.id) : null; return '<tr' + (b ? '' : ' style="background:#fff8ef"') + '><td class="mono">' + esc(o.id) + '</td><td class="mono">' + esc(o.tracking || '—') + '</td><td class="nowrap">' + dbr(o.createdAt) + '</td><td>' + (b ? '<span class="tag ok">bipado</span> <span class="footnote" style="margin:0">' + new Date(b.bipedAt).toLocaleString('pt-BR') + '</span>' : '<span class="tag warn">faltando</span>') + '</td><td>' + (st ? '<span class="tag">' + esc(st.label) + '</span>' : '—') + '</td><td><button class="btn-sm" data-goped360="' + esc(o.id) + '">Ficha do Pedido</button></td></tr>'; }).join('');
    ...
    // §17-21 Full/FBS: Tempo de Envio absorvido aqui como uma segunda visão da mesma tela — nunca
    // uma aba própria de novo. "Fila de bipagem" é o operacional do dia; "Prazos de envio" é o
    // mesmo teQueue()/shipDeadlineTs() de sempre, sem nenhuma alteração de cálculo.
    var viewToggle = '<div class="chips" style="margin-top:4px"><span class="chip' + (pedExpView === 'fila' ? ' chip-on' : '') + '" data-expview="fila">Fila de bipagem</span><span class="chip' + (pedExpView === 'prazos' ? ' chip-on' : '') + '" data-expview="prazos">Prazos de envio</span></div>';
    if (pedExpView === 'prazos') return head + expSessaoBlock() + pendenciasConferenciaBlock() + viewToggle + tempoEnvioBody();
    return head + expSessaoBlock() + pendenciasConferenciaBlock() + viewToggle + strip + bipe +
  ...
  function bindPedidosExpedicao() {
    ...
    app.querySelectorAll('[data-expview]').forEach(function (c) { c.onclick = function () { pedExpView = c.dataset.expview; render(); }; });
    if (pedExpView === 'prazos') { bindTempoEnvioBody(); return; }
  ...
  // §17-21 do prompt Full/FBS: Tempo de Envio deixa de ser aba própria e passa a viver DENTRO de
  // Expedição (nav vira Pedidos | Expedição | Dashboard | Importações) — mesma categorização de
  // vencidos/vence hoje/vence amanhã/próximos e o mesmo shipDeadlineTs() canônico, sem alterar nada.
  function tempoEnvioBody() {
    var head = '<div class="panel"><div class="ph"><h3>Prazos de Envio</h3><span class="footnote" style="margin:0">Evitar atraso e penalização da Shopee — prazo real vem da planilha (Data prevista de envio)</span></div></div>';
    var base = teQueue();
    ...
    var SORTS = {
      ...
      full_desc: function (a, b) { return (a.isFbs ? 1 : 0) - (b.isFbs ? 1 : 0); },
    ...
    return head + strip + classeChips +
      ...
      '<select class="select sm" id="tesort">' + [
        ...
        ['produto', 'Produto'], ['modalidade', 'Modalidade'], ['full', 'Full primeiro'], ['full_desc', 'Normal primeiro'], ['status', 'Status'],
  ...
  function pedidoComposicaoFinanceira(orderId) {
    ...
    if (mrRow) {
      ...
      addTaxa('Afiliado', mrRow.afiliado, [], 'Income', 'afiliado');
      ...
      addTaxa('Comissão (aproximado)', -Math.round((ord.commissionNet || 0) * 100), [], 'Pedidos — sem Income cruzado para este pedido', 'comissao');
      addTaxa('Taxa de serviço (aproximado)', -Math.round((ord.serviceFeeNet || 0) * 100), [], 'Pedidos — sem Income cruzado para este pedido', 'servico');
      addTaxa('Taxa de transação (aproximado)', -Math.round((ord.transactionFee || 0) * 100), [], 'Pedidos — sem Income cruzado para este pedido', 'transacao');
      addTaxa('Frete reverso (aproximado)', -Math.round((ord.reverseShippingFee || 0) * 100), [], 'Pedidos — sem Income cruzado para este pedido', 'envioReverso');
    }
    ...
    // consome .pendencia); mesma fórmula/tolerância que já era usada (§21 da Ficha).
    var pendencia = null;
    if (mrRow) {
      var diff = mrRow.liberado - (mrRow.preco + taxasSomaC);
      if (Math.abs(diff) > 100) pendencia = { id: 'PENDFIN-' + orderId, origem: 'PEDIDOS', orderId: orderId, tipo: 'COMPOSICAO_FINANCEIRA', diff: diff, liberado: mrRow.liberado, calculado: mrRow.preco + taxasSomaC, motivo: 'Diferença financeira de ' + brlC(diff) + ' entre a composição do pedido e o Pagamento Liberado — etapa: composição Shopee, motivo: lançamento não classificado' };
    }
    return { orderId: orderId, temIncome: !!mrRow, mrRow: mrRow, ord: ord, svcRows: svcRows, receitaC: receitaC, custoProdC: custoProdC, custoPendente: custoPendente, taxaRows: taxaRows, gTaxas: gTaxas, gDescontos: gDescontos, gCreditos: gCreditos, gOutros: gOutros, somaGrupo: somaGrupo, taxasCobradasC: somaGrupo(gTaxas), descontosComerciaisC: somaGrupo(gDescontos), creditosC: somaGrupo(gCreditos), outrosC: somaGrupo(gOutros), taxasSomaC: taxasSomaC, adjRows: adjRows, resultadoC: resultadoC, margemPct: margemPct, pendencia: pendencia };
  }
  ...
  function openPedidoFicha360(orderId) {
    ...
    var gTaxas = comp.gTaxas, gDescontos = comp.gDescontos, gCreditos = comp.gCreditos, gOutros = comp.gOutros;
  ...
  var WSITUACAO = { CONCILIADO: '🟢 Conciliado — origem exata', PROVAVEL: '🟡 Origem provável', CREDITO_SEM_ORIGEM: '🟡 Crédito sem origem identificada', FORA_COBERTURA: '⚪ Fora da cobertura das fontes', SEM_ORIGEM: '🔴 Sem origem', DIVERGENTE: '🔴 Valor divergente', DUPLICIDADE: '🔴 Possível duplicidade', DESCONTO_INDEVIDO: '🔴 Possível desconto indevido', MANUAL_PENDENTE: '🟠 Classificação manual pendente' };
  ...
  function caixaRegistrarTransferenciaBanco(valor, quandoISO, conta, observacao) {
    var rec = { id: 'BT-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8), valor: r2(valor), quando: quandoISO, conta: conta || null, observacao: observacao || null, criadoPor: 'Operador', createdAt: new Date().toISOString() };
  ...
  function openCaixaFechamentoDia(dateKey) {
    ...
    function body() {
      ...
      // Camada 1 — Saldo do Dia: recebido via Acelera (rastreável até resgate→pedidos, nunca digitado
      // à mão) + transferências para o banco (evento manual PURO Carteira→Banco — nunca altera
      // Acelera/Pedido/receita/lucro). São duas movimentações SEPARADAS, nunca somadas/subtraídas
      // silenciosamente uma da outra a menos que ambas existam de fato neste dia (§87).
      ...
      var blocoSaldo = '<div class="panel"><div class="ph"><h3>0. Saldo do Dia</h3></div><div class="pb">' +
        (ac.n ? (kv('Recebido via Acelera (bruto antecipado)', brlC(ac.antec)) + kv('Taxa Acelera', brlC(ac.taxa)) + kv('Valor líquido recebido', brlC(ac.liquido))) : '<div class="footnote" style="padding:0 0 10px">Nenhum resgate do Acelera consolidado nesta data.</div>') +
        '<div style="margin-top:8px"><b>Transferências para o banco registradas neste dia</b></div>' +
        ...
        '<div><label class="fld">Conta / banco (opcional)</label><input class="input sm" id="bt-conta" style="width:160px"></div>' +
      ...
      var pendCartTable = '<div class="panel"><div class="ph"><h3>6. Pendências da Carteira</h3><span class="footnote" style="margin:0">' + nn(pendCart.total) + ' no total</span></div><div class="table-wrap"><table class="report"><thead><tr><th>Pedido</th><th>Problema</th><th>Valor</th><th></th></tr></thead><tbody>' + (pendCartRows || '<tr><td colspan="4" class="empty">Nenhuma pendência da carteira neste dia. 🎉</td></tr>') + '</tbody></table></div></div>';
      // §68-72: checklist visual das duas famílias de pendência ANTES de fechar — nunca esconde.
      var checklist = '<div class="panel"><div class="ph"><h3>Checklist de fechamento</h3></div><div class="pb"><div class="kstrip" style="flex-wrap:wrap">' +
      ...
      // Ordem §82 (impressão/PDF): Saldo do dia → Fluxo (Pedidos/Expedição/Acelera/Pendências) →
      // Carteira → DRE (Lucro/Margem) → Situação/Responsável (footer).
      return '<div class="dh"><div><b>FECHAMENTO — ' + esc(dbr(dateKey)) + '</b> <span class="tag ' + lbl[1] + '" style="margin-left:6px">' + lbl[0] + '</span>' + (st.status === 'REVISAO_NECESSARIA' ? ' <span class="footnote" style="margin:0">⚠ movimento posterior a este fechamento</span>' : '') + '</div><button class="x">&times;</button></div><div class="dbd">' + blocoSaldo + blocoVendas + blocoExp + blocoAc + blocoFin + pendTable + pendCartTable + dreBlock + checklist + footer + '</div>';
    }
    function wire() {
      panel.querySelector('.x').onclick = function () { d.remove(); };
      ...
      var btReg = panel.querySelector('#bt-registrar'); if (btReg) btReg.onclick = function () {
        ...
        var conta = panel.querySelector('#bt-conta').value.trim() || null; var obs = panel.querySelector('#bt-obs').value.trim() || null;
        caixaRegistrarTransferenciaBanco(valor, new Date(quando).toISOString(), conta, obs).then(function () { toast('Transferência registrada', brl(valor)); refresh(); render(); });
      };
