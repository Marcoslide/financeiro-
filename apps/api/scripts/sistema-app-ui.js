...
(function () {
  ...
  function caixaDayTransferenciasBancarias(dateKey) {
    var autoTxs = wallet.filter(function (t) { return t.origin === 'SHOPEE' && t.date && t.date.slice(0, 10) === dateKey && wIsTransferenciaBancaria(t); });
  ...
  function caixaDaySaldoCarteira(dateKey) {
    var antes = wallet.filter(function (t) { return t.balance != null && t.date && t.date.slice(0, 10) < dateKey; }).sort(function (a, b) { return (a.date || '').localeCompare(b.date || ''); });
    var saldoInicial = antes.length ? antes[antes.length - 1].balance : null;
    var doDia = wallet.filter(function (t) { return t.balance != null && t.date && t.date.slice(0, 10) === dateKey; }).sort(function (a, b) { return (a.date || '').localeCompare(b.date || ''); });
  ...
  function bankAccountSave(rec) {
    ...
    var full = { id: id, nome: rec.nome, banco: rec.banco || '', agencia: rec.agencia || '', conta: rec.conta || '', tipo: rec.tipo || '', titular: rec.titular || '', documento: rec.documento || '', observacao: rec.observacao || '', ativa: rec.ativa !== false, createdAt: rec.createdAt || new Date().toISOString() };
  ...
  function openNovaOperacaoModal() {
    ...
    function render2() {
      ...
      panel.innerHTML = '<div class="dh"><div><b>Nova operação</b></div><button class="x">&times;</button></div><div class="dbd">' +
        ...
        '<div id="no-empresa-nova" style="display:' + (companies.length ? 'none' : 'block') + '">' +
        ...
        '<label class="fld">CNPJ</label><input class="input" id="no-cnpj" style="width:100%">' +
        '</div>' +
      ...
      empresaSel.onchange = function () { novaDiv.style.display = empresaSel.value === '__nova__' ? 'block' : 'none'; };
      panel.querySelector('#no-salvar').onclick = function () {
        ...
        if (empresaSel.value === '__nova__') {
          var razao = panel.querySelector('#no-razao').value.trim(); if (!razao) { toast('Informe a razão social da nova empresa', '', true); return; }
          companySave({ razaoSocial: razao, nomeFantasia: panel.querySelector('#no-fantasia').value.trim() || razao, cnpj: panel.querySelector('#no-cnpj').value.trim() }).then(function (c) { proceed(c.id); });
  ...
  function openCaixaFechamentoDia(dateKey) {
    ...
    function body() {
      ...
      var bancoOptions = '<option value="">Selecione a conta…</option>' + bankAccounts.filter(function (b) { return b.ativa !== false; }).map(function (b) { return '<option value="' + esc(b.id) + '">' + esc(b.nome) + (b.banco ? ' — ' + esc(b.banco) : '') + '</option>'; }).join('') + '<option value="__novo__">+ Cadastrar nova conta</option>';
