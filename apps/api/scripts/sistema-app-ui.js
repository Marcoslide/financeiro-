...
(function () {
  ...
  function renderDashboard() {
    ...
    app.querySelectorAll('[data-dashgo]').forEach(function (b) { b.onclick = function () {
      ...
      else if (go === 'carteirapend') { route = 'carteira'; walletSub = 'pendencias'; }
      render();
    }; });
  }
  ...
  function openPedidoFicha360(orderId) {
    ...
    // ---- §45-47: Devolução REABRE o resultado do pedido — nunca apaga o histórico original. Mostra
    // a evolução Resultado original (antes da devolução) → Evento → Resultado atualizado. Não é um
    // cálculo novo: resultadoAntes = resultadoC + devImpactoCents (o mesmo devImpactoCents já somado
    // no Resultado final abaixo, só somado de volta para reconstruir o "antes").
    var devReaberturaBlock = '';
    if (occs.length && resultadoC != null) {
      var resultadoAntes = r2(resultadoC + devImpactoCents);
      if (Math.abs(devImpactoCents) > 1) {
  ...
  var walletPendF = { status: '' };
  function walletPendItems(m) {
    var g = walletOrigemDiag(m); var seen = {}; var items = [];
    function add(t, motivo, detalhe) { if (seen[t.id]) return; seen[t.id] = true; items.push({ t: t, motivo: motivo, detalhe: detalhe }); }
    g.divergentes.forEach(function (x) { add(x.t, 'Diferença de valor — ' + x.origin.fonte, 'Esperado ' + brl(x.origin.esperado) + ' · Carteira ' + brl(x.t.amount) + ' · Diferença ' + brl(x.origin.diff)); });
    g.dup.forEach(function (d) { d.itens.forEach(function (t) { add(t, 'Possível cobrança duplicada — ' + d.fonte, nn(d.itens.length) + '× o mesmo valor (' + brl(d.valor) + ') na carteira'); }); });
    g.semOrigem.forEach(function (t) { add(t, 'Origem não identificada (' + wcatLabel(wEffCat(t)) + ')', 'Fonte relevante já cobre esta data e mesmo assim não achou correspondência'); });
    var diag = walletDiag(m);
    diag.divergentes.forEach(function (d) { add(d.tx, 'Diferença de saldo do extrato (reconciliação matemática)', 'Esperado ' + brl(d.expected) + ' · Informado ' + brl(d.informed) + ' · Diferença ' + brl(d.gap)); });
    return items;
  }
  function walletPendencias() {
    var m = walletMetrics(); var items = walletPendItems(m);
    var counts = {}; items.forEach(function (it) { var s = wStatus(it.t); counts[s] = (counts[s] || 0) + 1; });
    var head = secHead('CARTEIRA · CENTRAL DE PENDÊNCIAS', 'O que ainda precisa de decisão', 'Junta tudo que a conciliação automática não conseguiu fechar sozinha — diferença de valor, possível duplicidade e origem não identificada — com o mesmo status interno usado no resto da Carteira (nunca apagado ao reimportar).');
    var strip = kstrip(Object.keys(WSTATUS).map(function (k) { return { l: WSTATUS[k], v: nn(counts[k] || 0), cls: k === 'RESOLVIDO' ? 'green' : (k === 'NAO_REVISADO' && counts[k] ? 'amber' : '') }; }).concat([{ l: 'Total de pendências', v: nn(items.length), cls: items.length ? 'red' : 'green' }]));
    var chips = '<div class="chips" style="margin-top:8px"><span class="chip' + (!walletPendF.status ? ' chip-on' : '') + '" data-pendst="">Todas (' + nn(items.length) + ')</span>' + Object.keys(WSTATUS).map(function (k) { return '<span class="chip' + (walletPendF.status === k ? ' chip-on' : '') + '" data-pendst="' + k + '">' + WSTATUS[k] + ' (' + nn(counts[k] || 0) + ')</span>'; }).join('') + '</div>';
    var filtered = walletPendF.status ? items.filter(function (it) { return wStatus(it.t) === walletPendF.status; }) : items;
    var rows = filtered.slice(0, 300).map(function (it) { return '<tr class="rowlink" data-wtx="' + esc(it.t.id) + '"><td class="nowrap">' + dbr(it.t.date) + '</td><td class="cell-text">' + esc(it.motivo) + '</td><td class="cell-text">' + esc(it.detalhe) + '</td><td class="nowrap">' + brl(it.t.amount) + '</td><td><span class="tag ' + (wStatus(it.t) === 'RESOLVIDO' || wStatus(it.t) === 'EXPLICADO' ? 'ok' : 'neutral') + '">' + esc(WSTATUS[wStatus(it.t)]) + '</span></td></tr>'; }).join('');
    var table = '<div class="panel"><div class="table-wrap"><table class="report"><thead><tr><th>Data</th><th>Motivo</th><th>Detalhe</th><th>Valor</th><th>Status</th></tr></thead><tbody>' + (rows || '<tr><td colspan="5" class="empty">Nenhuma pendência neste filtro. 🎉</td></tr>') + '</tbody></table></div></div>';
    return head + strip + chips + '<div class="count-line" style="margin-top:8px"><b>' + nn(filtered.length) + '</b> pendência(s)</div>' + table;
  }
  function bindWalletPendencias() {
    app.querySelectorAll('[data-pendst]').forEach(function (c) { c.onclick = function () { walletPendF.status = c.dataset.pendst; render(); }; });
  }
  ...
  function renderCarteira() {
    ...
    try {
      ...
      else if (walletSub === 'pendencias') body.innerHTML = walletPendencias();
    ...
    if (walletSub === 'pendencias') bindWalletPendencias();
  ...
  function renderMinhaRenda() {
    ...
    app.querySelectorAll('[data-mrgocarteira]').forEach(function (b) { b.onclick = function () { route = 'carteira'; walletSub = 'pendencias'; render(); }; });
  ...
  function mrVisao() {
    ...
    return head + strip1 + mrQualidadeConciliacaoBox() + strip2 + strip3 + coverage + mrWaterfall(t) + cobertura + tendLink + dedup + shipBox + mrAlertas();
  }
