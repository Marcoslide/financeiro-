...
(function () {
  ...
  function autoReceiptFromTracking(o) { var ts = (o.trackingStatus || '').normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase(); var concluded = /conclu|entregue ao remetente|recebid.*loja|retirada bem/.test(ts) && /devolu|retorn|conclu|retirada/.test(ts); if (concluded && ['RECEBIDO', 'PARCIAL', 'DIVERGENCIA', 'CHEGOU_CONFERIR'].indexOf(o.receiptState) < 0) { o.receiptState = 'CHEGOU_CONFERIR'; addActivity(o, 'RECEIPT', { message: 'Shopee informou conclusão do retorno — pronto para conferência (baixa manual)', userName: 'Sistema' }); } }
  ...
  function planCard(p) {
    ...
    return '<div class="panel"><div class="ph"><h3>' + esc(p.title) + '</h3><span class="tag ' + (p.status === 'ENCERRADO' ? 'ok' : p.status === 'MEDINDO' ? 'info' : 'neutral') + '">' + (PLAN_STATUS[p.status] || p.status) + '</span></div><div class="pb">' +
      ...
      (p.implementedAt ? '<div class="fin-line"><span>Implantado em</span><span>' + dbr(p.implementedAt) + '</span></div>' +
        '<div class="footnote" style="margin:6px 0 4px">Acompanhamento (janela pós × mesma janela antes)</div>' +
        '<div class="table-wrap"><table class="report"><thead><tr><th>Janela</th><th>Antes</th><th>Depois</th><th>Δ</th><th>Observado</th></tr></thead><tbody>' +
        planWindows(p).map(function (w) { var higher = !!INDICATOR_HIGHER[p.indicatorKind]; var delta = (w.pre != null && w.post != null) ? (higher ? w.post - w.pre : w.pre - w.post) : null; var dcls = delta == null ? '' : delta > 0 ? 'pos' : delta < 0 ? 'neg' : ''; return '<tr><td>' + w.label + '</td><td>' + (w.pre != null ? fmtInd(p, w.pre) : '—') + '</td><td>' + (w.post != null ? fmtInd(p, w.post) : '—') + '</td><td class="' + dcls + '">' + (delta == null ? '—' : (delta > 0 ? '▼ melhora' : delta < 0 ? '▲ piora' : '=')) + '</td><td class="footnote" style="margin:0">' + (w.obs ? w.obs + 'd' + (w.preliminary ? ' · preliminar' : '') : 'sem dados') + '</td></tr>'; }).join('') + '</tbody></table></div>' +
        '<div class="footnote" style="margin:4px 0 0">Maturidade</div><div style="height:10px;border-radius:6px;background:var(--line);overflow:hidden;margin-top:3px"><div style="height:100%;width:' + mat + '%;background:var(--info)"></div></div><div class="footnote" style="margin:2px 0 8px">' + mat + '% da janela de ' + (p.windowAfterDays || 30) + ' dias — ' + (mat < 100 ? 'resultado preliminar' : 'janela completa') + '</div>' : '') +
