...
(function () {
  ...
  function walletOrigin(t) {
    ...
    if (cat === 'ACELERA' && acelera.length) {
      ...
      if (cand) return { tier: 'Valor + janela de data', confidence: 'alta', fonte: 'Resgate ' + cand.resgate, esperado: cand.receb / 100, encontrado: t.amount, diff: 0, ok: true };
    }
    return null;
  }
  ...
  function walletOrigemDiag(m) {
    ...
    var comOrigem = [], semOrigem = [];
    elegiveis.forEach(function (t) { var o = walletOrigin(t); if (o) comOrigem.push({ t: t, origin: o }); else semOrigem.push(t); });
    var altaConf = comOrigem.filter(function (x) { return x.origin.confidence === 'alta'; });
    ...
    return { elegiveis: elegiveis.length, conciliados: conciliados, divergentes: divergentes, estimativa: estimativa, semOrigem: semOrigem, dup: dup };
  }
  function walletOrigemPanel(m) {
    var g = walletOrigemDiag(m);
    if (!g.elegiveis) return '';
    var strip = kstrip([
      { l: '🟢 Origem conciliada (fonte definitiva)', v: nn(g.conciliados.length), cls: 'green' },
      { l: '🔴 Diferença de valor (fonte definitiva)', v: nn(g.divergentes.length), cls: g.divergentes.length ? 'red' : 'green' },
      ...
      { l: '⚪ Sem origem encontrada', v: nn(g.semOrigem.length), cls: g.semOrigem.length ? 'amber' : 'green', s: nn(g.elegiveis) + ' elegíveis (Acelera/Devolução/Indenização/Renda)' },
    ...
    var semTable = g.semOrigem.length ? '<div class="panel"><div class="ph"><h3>⚪ Pendência — origem não identificada</h3><span class="footnote" style="margin:0">o sistema procurou em Acelera, Devolução, Indenização e Minha Renda e não encontrou correspondência — pode ser porque o pedido está fora do período importado nos outros módulos</span></div><div class="table-wrap"><table class="report"><thead><tr><th>Data</th><th>Categoria</th><th>Descrição</th><th>Pedido</th><th>Valor</th></tr></thead><tbody>' + semRows + '</tbody></table></div></div>' : '';
    ...
    var nota = callout('', 'Origem automática', 'Para cada lançamento (Acelera, Devolução, Indenização, Renda), o sistema procura a origem esperada no próprio sistema por ID do pedido — e, quando o lançamento é um resgate inteiro do Acelera (sem ID de pedido único), por valor + janela de data. Acelera e Minha Renda/Income são fontes definitivas (🟢/🔴); Devolução e Compensação usam o valor solicitado, ainda provisório até a baixa — por isso aparecem à parte, nunca como divergência. Nunca soma o mesmo evento duas vezes: a carteira só confirma se o evento esperado aconteceu.');
    return nota + strip + divTable + estTable + semTable + dupTable;
  }
