...
(function () {
  ...
  function openCaixaFechamentoDia(dateKey) {
    ...
    function body() {
      ...
      var vendas = caixaDayVendas(dateKey), exp = caixaDayExpedicao(dateKey), ac = aceleraResultadoFechamento(dateKey), pend = st.pend, pendCart = st.pendCart;
      var margem = vendas.valor ? r2(vendas.lucroC / 100 / vendas.valor * 100) : null;
      ...
      // Fechamento organizado em 10 blocos (rodada CAIXA/PIX/MULTI-CNPJ): 1.Resumo do Dia (enxuto) →
      // 2.Acelera (+ conferência Expedição×Acelera e Receitas Adicionais, colapsadas) → 3.Conciliação
      // do Dinheiro → 4.Descontos e Créditos Posteriores (por natureza) → 5.Transferências Bancárias
      // (Pix/saque reconhecidos automaticamente + cadastro de conta) → 6.DRE → 7.Pedidos [recolhível]
      // → 8.Expedição [recolhível] → 9.Pendências [recolhível] → 10.Fechar. O Caixa NUNCA reconstrói
      // o que é do Acelera/Carteira — só lê os motores canônicos já aprovados.
      var mov = caixaDayMovimentosCarteira(dateKey);
      var conc = caixaDayLancamentosConciliados(dateKey); var pc = conc.porCategoria;
      var transf = caixaDayTransferenciasBancarias(dateKey);
      var saldoCart = caixaDaySaldoCarteira(dateKey);
      var movPost = caixaDayMovimentosPosteriores(dateKey); var pcat = movPost.porCategoria;
      var outrasReceitasValor = ((pc.PENDENCIA_ANTERIOR ? pc.PENDENCIA_ANTERIOR.valor : 0) + (pc.COMPENSACAO ? pc.COMPENSACAO.valor : 0) + (pc.OUTRAS_RECEITAS ? pc.OUTRAS_RECEITAS.valor : 0));
      // Pendência não explicada do dia = pendências da Carteira (débito/crédito sem origem, valor
      // divergente, duplicidade) — soma dos valores em módulo, nunca inventa explicação.
      ...
      // 1. RESUMO DO DIA — enxuto e executivo (§C/§D do complemento): nunca dezenas de números no
      // topo. Créditos/Débitos/Saldo remanescente saem daqui (continuam no bloco 3, detalhados).
      var resumoTop = '<div class="panel"><div class="ph"><h3>1. Resumo do Dia</h3></div><div class="pb" style="padding-top:0">' +
        kstrip([
          { l: 'Bruto Acelera', v: ac.resgatesN ? brlC(ac.valorBruto) : '—', cls: 'blue' },
          { l: 'Taxa Acelera', v: ac.resgatesN ? brlC(-Math.abs(ac.taxaAcelera)) : '—', cls: 'red' },
          { l: 'Recebido do Acelera', v: ac.resgatesN ? brlC(ac.valorLiquido) : '—', cls: 'green' },
          { l: 'Transferido para banco', v: brl(transf.total), cls: transf.total ? 'amber' : 'blue' },
          { l: 'Pendência não explicada', v: brl(pendNaoExplicadaValor), cls: pendNaoExplicadaValor ? 'red' : 'green' }
        ]) +
        '</div></div>';
      // 2. ACELERA — histórico da antecipação NUNCA muda por causa de movimento posterior da Carteira
      // (§83/§85): valor bruto/taxa/líquido continuam sendo os do Acelera, resgate a resgate, ID
      // sempre visível. "Ver pedidos do Acelera" nunca esconde a lista atrás de um total. Conferência
      // Expedição×Acelera e Receitas Adicionais entram como sub-seções colapsadas (mesmo motor,
      // menos blocos soltos no topo).
      var resgateBlocks = ac.resgates.map(function (r) {
        var statusTxt = Object.keys(r.statusCounts).map(function (k) { return k + ': ' + nn(r.statusCounts[k]); }).join(' · ');
        var pedidosTbl = r.pedidos.map(function (p) { var info = exp[p]; return '<tr><td class="mono">' + esc(p) + '</td><td>' + (info ? ((info.isFbs ? 'Full confirmado' : 'Bipado') + (info.at ? (' em ' + dbr(info.at)) : '')) : '🔴 sem confirmação de expedição') + '</td></tr>'; }).join('');
        return '<details class="panel" style="margin-top:8px"><summary style="cursor:pointer;padding:10px 14px;font-weight:700">Resgate ' + esc(r.resgate) + ' — ' + nn(r.n) + ' pedido(s) · líquido ' + brlC(r.liquido) + (r.divergente ? ' <span class="tag warn">líquido divergente</span>' : '') + '</summary><div class="pb">' +
          kv('Quantidade de pedidos', nn(r.n)) + kv('Valor bruto', brlC(r.bruto)) + kv('Taxa Acelera', brlC(r.taxa)) + kv('Valor líquido', brlC(r.liquido)) + kv('Alíquota efetiva', r.aliqLabel) + kv('Status (Shopee)', statusTxt || '—') +
          '<div class="footnote" style="margin:6px 0 2px">Ver pedidos do Acelera (' + nn(r.pedidos.length) + ')</div><div class="table-wrap"><table class="report"><thead><tr><th>Pedido</th><th>Expedição</th></tr></thead><tbody>' + (pedidosTbl || '<tr><td colspan="2" class="empty">—</td></tr>') + '</tbody></table></div>' +
          '</div></details>';
      }).join('');
      var confPedidosSet = {}; ac.expedidosEsperados.forEach(function (p) { confPedidosSet[p] = 1; }); ac.pedidosAcelera.forEach(function (p) { confPedidosSet[p] = 1; });
      var confPedidos = Object.keys(confPedidosSet);
      var confRows = confPedidos.slice(0, 300).map(function (p) {
        var info = exp[p]; var pv = ac.pedidoValores[p]; var noAcelera = ac.pedidosAcelera.indexOf(p) >= 0;
        var situacao = (info && noAcelera) ? '🟢 OK' : (info && !noAcelera ? '🔴 Não localizado no Acelera' : (!info && noAcelera ? '🔴 Sem confirmação de expedição' : '—'));
        return '<tr><td class="mono">' + esc(p) + '</td><td>' + (info ? (info.at ? dbr(info.at) : 'confirmado') : '—') + '</td><td>' + (noAcelera ? 'sim' : 'não') + '</td><td class="nowrap">' + (pv ? brlC(pv.bruto) : '—') + '</td><td class="nowrap">' + (pv ? brlC(pv.taxa) : '—') + '</td><td class="nowrap">' + (pv ? brlC(pv.liquido) : '—') + '</td><td>' + situacao + '</td></tr>';
      }).join('');
      var conferenciaBlock = '<details class="panel" style="margin-top:8px"><summary style="cursor:pointer;padding:10px 14px;font-weight:700">Conferência Expedição × Acelera</summary><div class="pb">' +
        kstrip([
          { l: 'Expedidos esperados', v: nn(ac.expedidosEsperadosN), cls: 'blue' },
          { l: 'Encontrados no Acelera', v: nn(ac.expedidosEncontradosN), cls: 'green' },
          { l: 'Não localizados', v: nn(ac.expedidosNaoEncontrados.length), cls: ac.expedidosNaoEncontrados.length ? 'red' : 'green' },
          { l: 'Acelera sem confirmação interna', v: nn(ac.aceleraSemConfirmacaoExpedicao.length), cls: ac.aceleraSemConfirmacaoExpedicao.length ? 'amber' : 'green' },
          { l: 'Divergências de valor', v: nn(ac.divergencias.length), cls: ac.divergencias.length ? 'red' : 'green' },
          { l: 'Pendências abertas', v: nn(ac.pendencias.length), cls: ac.pendencias.length ? 'red' : 'green' }
        ]) +
        (ac.expedidosNaoEncontrados.length ? callout('warn', '🔴 ' + nn(ac.expedidosNaoEncontrados.length) + ' pedido(s) expedido(s) ainda não pago(s)', 'Valor esperado (preço do pedido, referência — não é o valor que o Acelera vai antecipar): ' + brl(ac.expedidosNaoEncontrados.reduce(function (s, oid) { var o = orders.find(function (x) { return x.id === oid; }); return s + (o ? o.totalAmount || 0 : 0); }, 0))) : '') +
        (ac.pedidosAntigosRecebidosHoje.length ? kv('Recebidos de pendências anteriores', nn(ac.pedidosAntigosRecebidosHoje.length) + ' pedido(s) · ' + brlC(ac.valorAntigosRecebidosHoje)) : '') +
        '<details style="margin-top:8px"><summary style="cursor:pointer;font-weight:700;padding:6px 0">Ver conferência dos pedidos (' + nn(confPedidos.length) + ')</summary><div class="table-wrap" style="margin-top:8px"><table class="report"><thead><tr><th>Pedido</th><th>Expedição</th><th>Acelera</th><th>Bruto</th><th>Taxa</th><th>Líquido</th><th>Situação</th></tr></thead><tbody>' + (confRows || '<tr><td colspan="7" class="empty">—</td></tr>') + '</tbody></table></div></details>' +
        '</div></details>';
      var receitaRows = ['PENDENCIA_ANTERIOR', 'COMPENSACAO', 'OUTRAS_RECEITAS'].map(function (k) {
        var g = pc[k]; if (!g) return '';
        return g.itens.map(function (it) { return '<tr class="rowlink" data-wtx="' + esc(it.t.id) + '"><td>' + esc(CAIXA_CART_CAT_LABEL[k]) + '</td><td class="mono">' + esc(wOrderId(it.t) || '—') + '</td><td class="cell-text">' + esc((it.t.desc || it.t.tipo || '—').slice(0, 40)) + '</td><td class="nowrap pos">' + brl(it.t.amount) + '</td><td class="cell-text">' + esc(it.origem) + '</td><td><span class="tag ok">🟢 conciliado</span></td></tr>'; }).join('');
      }).join('');
      var receitasAdicionaisBlock = '<details class="panel" style="margin-top:8px"><summary style="cursor:pointer;padding:10px 14px;font-weight:700">Receitas Adicionais (' + brl(outrasReceitasValor) + ')</summary><div class="pb"><div class="footnote" style="margin:0 0 6px">nunca somado ao líquido do Acelera — dinheiro adicional já explicado</div>' +
        '<div class="table-wrap"><table class="report"><thead><tr><th>Tipo</th><th>Pedido</th><th>Descrição</th><th>Valor</th><th>Origem</th><th>Situação</th></tr></thead><tbody>' + (receitaRows || '<tr><td colspan="6" class="empty">Nenhuma receita adicional conciliada neste dia.</td></tr>') + '</tbody></table></div>' +
        '</div></details>';
      var bloco2 = '<div class="panel"><div class="ph"><h3>2. Acelera</h3><span class="footnote" style="margin:0">' + nn(ac.resgatesN) + ' resgate(s) · ' + nn(ac.pedidosAceleraN) + ' pedido(s) antecipado(s)</span></div><div class="pb">' +
        (ac.resgatesN ? (kv('Resgates', nn(ac.resgatesN)) + kv('Pedidos antecipados', nn(ac.pedidosAceleraN)) + kv('Valor bruto resgatado', brlC(ac.valorBruto)) + kv('Taxa Acelera', brlC(ac.taxaAcelera)) + kv('Valor líquido do Acelera', brlC(ac.valorLiquido)) + resgateBlocks) : '<div class="footnote" style="padding:0 0 10px">Nenhum resgate do Acelera consolidado nesta data.</div>') +
        conferenciaBlock + receitasAdicionaisBlock +
        '</div></div>';
      // 3. CONCILIAÇÃO DO DINHEIRO — equação explícita (§O/§P do complemento): nunca "saldo
      // remanescente" solto, sempre a conta completa + comparação com o saldo informado pela Shopee
      // quando a planilha da Carteira trouxer saldo por linha.
      var bloco3 = '<div class="panel"><div class="ph"><h3>3. Conciliação do Dinheiro</h3></div><div class="pb">' +
        ...
        '<div class="fin-line total"><span>= Saldo final calculado da Carteira</span><b>' + (saldoCart.saldoFinalCalculado != null ? brl(saldoCart.saldoFinalCalculado) : 'sem saldo inicial conhecido') + '</b></div>' +
        (saldoCart.saldoFinalInformado != null ? ('<div class="fin-line"><span>Saldo final informado (Carteira)</span><span>' + brl(saldoCart.saldoFinalInformado) + '</span></div>' +
          '<div class="fin-line' + (saldoCart.diferenca && Math.abs(saldoCart.diferenca) > 1 ? '' : ' total') + '"><span>Diferença' + (saldoCart.diferenca && Math.abs(saldoCart.diferenca) > 1 ? ' <span class="tag warn">🔴 sem explicação</span>' : ' <span class="tag ok">🟢 confere</span>') + '</span><b class="' + (saldoCart.diferenca < 0 ? 'neg' : 'pos') + '">' + brl(saldoCart.diferenca) + '</b></div>') : '<div class="footnote" style="margin-top:6px">Planilha da Carteira sem coluna de saldo por linha neste período — só a conta calculada está disponível.</div>') +
        '</div></div>';
      // 4. DESCONTOS E CRÉDITOS POSTERIORES — agrupado por NATUREZA (§K/§L/§AG/§AH do complemento):
      // "Ajuste do Shopee Acelera ID do pedido..." NUNCA é a taxa do resgate (já descontada em
      // bruto−taxa=líquido) — é um evento novo, mostrado aqui separado, com pedido e motivo sempre
      // visíveis. Pendências continuam resolvíveis pelo mesmo motor (openWalletTx) — resolver nunca
      // apaga, só muda a categoria/situação (§1/§25/§26 já implementado).
      var MOVPOST_ORDER = ['AJUSTES_ACELERA', 'DEVOLUCOES_REEMBOLSOS', 'AJUSTES_SHOPEE', 'CREDITOS_COMPENSACOES', 'OUTROS_CREDITOS', 'OUTROS_DEBITOS', 'TRANSFERENCIAS_BANCARIAS', 'NAO_IDENTIFICADOS'];
      var movPostGroups = MOVPOST_ORDER.map(function (k) {
        var g = pcat[k]; if (!g || !g.itens.length) return '';
        var rows = g.itens.slice(0, 200).map(function (t) {
          return '<tr class="rowlink" data-wtx="' + esc(t.id) + '"><td class="nowrap">' + dbr(t.date) + '</td><td class="mono">' + esc(wOrderId(t) || '— Não identificado') + '</td><td class="cell-text">' + esc((t.desc || t.tipo || '—').slice(0, 60)) + '</td><td class="nowrap ' + (t.amount < 0 ? 'neg' : 'pos') + '">' + brl(t.amount) + '</td><td class="cell-text">' + esc(wProvavelOrigem(t)) + '</td><td>' + caixaCartSituacao(t) + '</td></tr>';
        }).join('');
        return '<details class="panel" style="margin-top:8px"><summary style="cursor:pointer;padding:10px 14px;font-weight:700">' + esc(g.label) + ' — ' + nn(g.n) + ' movimento(s) · <b class="' + (g.valor < 0 ? 'neg' : 'pos') + '">' + brl(g.valor) + '</b></summary><div class="pb"><div class="table-wrap"><table class="report"><thead><tr><th>Data</th><th>Pedido</th><th>Descrição Shopee</th><th>Valor</th><th>Origem</th><th>Situação</th></tr></thead><tbody>' + rows + '</tbody></table></div></div></details>';
      }).join('');
      var bloco4 = '<div class="panel"><div class="ph"><h3>4. Descontos e Créditos Posteriores</h3><span class="footnote" style="margin:0">' + nn(movPost.todos.length) + ' movimento(s) · ' + nn(pendCart.total) + ' pendência(s)</span></div><div class="pb">' +
        (movPostGroups || '<span class="tag neutral">Nenhum movimento posterior ao recebimento do Acelera neste dia.</span>') +
        '</div></div>';
      // 5. TRANSFERÊNCIAS BANCÁRIAS — Pix/Saque/Transferência bancária JÁ IMPORTADOS da Carteira
      // contam automaticamente (§E-H/§V-Y do complemento); registro manual continua existindo para
      // contas/casos sem Carteira importada, com aviso de possível duplicidade (nunca soma às cegas).
      var transfAutoRows = transf.autoTxs.map(function (t) { return '<tr class="rowlink" data-wtx="' + esc(t.id) + '"><td class="nowrap">' + dbr(t.date) + '</td><td class="cell-text">' + esc((t.desc || t.tipo || '—').slice(0, 50)) + '</td><td class="nowrap">' + brl(Math.abs(t.amount)) + '</td><td><span class="tag ok">🟢 Carteira Shopee</span></td></tr>'; }).join('');
      var transfManualRows = transf.manualComDuplicidade.map(function (x) { var t = x.t; return '<div class="fin-line"><span>' + new Date(t.quando).toLocaleString('pt-BR') + (t.conta ? ' · ' + esc(t.conta) : '') + (t.observacao ? ' <span class="footnote" style="margin:0">' + esc(t.observacao) + '</span>' : '') + (x.possivelDuplicado ? ' <span class="tag warn">⚠ possível duplicidade de um Pix já importado da Carteira</span>' : '') + '</span><b>' + brl(t.valor) + '</b></div>'; }).join('');
      var bancoOptions = '<option value="">Selecione a conta…</option>' + bankAccountsForActiveCompany().filter(function (b) { return b.ativa !== false; }).map(function (b) { return '<option value="' + esc(b.id) + '">' + esc(b.nome) + (b.banco ? ' — ' + esc(b.banco) : '') + '</option>'; }).join('') + '<option value="__novo__">+ Cadastrar nova conta</option>';
      var bloco5 = '<div class="panel"><div class="ph"><h3>5. Transferências Bancárias</h3><span class="footnote" style="margin:0">total do dia: ' + brl(transf.total) + '</span></div><div class="pb">' +
        (transf.autoTxs.length ? ('<div style="margin:4px 0"><b>Reconhecidas automaticamente na Carteira (Pix/Saque/Transferência)</b></div><div class="table-wrap"><table class="report"><thead><tr><th>Data</th><th>Descrição</th><th>Valor</th><th>Origem</th></tr></thead><tbody>' + transfAutoRows + '</tbody></table></div>') : '<div class="footnote" style="margin:4px 0">Nenhum Pix/Saque/Transferência reconhecido na Carteira para este dia.</div>') +
        '<div style="margin:12px 0 4px"><b>Registros manuais</b></div>' +
        (transfManualRows || '<div class="footnote" style="margin:4px 0">Nenhuma transferência manual registrada para este dia.</div>') +
        '<div class="fin-line total"><span>Total transferido (automático + manual)</span><b>' + brl(transf.total) + '</b></div>' +
        ...
        (!bankAccounts.length ? '<span class="footnote" style="margin:0">Nenhuma conta cadastrada — use "+ Cadastrar nova conta" no seletor acima.</span>' : '') +
        '</div></div>' +
        '</div></div>';
      // 6. DRE — motor nativo do Caixa, nomes reais, mesma Receita Bruta canônica do topo (§AA/§AC).
      var bloco6 = caixaDreBlock(caixaDreDia(dateKey));
      // 7. PEDIDOS DO DIA [recolhível] — nunca trunca: todos os pedidos pagos aparecem ao expandir.
      var pedRows7 = vendas.pedidos.map(function (o) { var c = pedidoComposicaoFinanceira(o.id); return '<tr class="rowlink" data-goped360="' + esc(o.id) + '"><td class="mono">' + esc(o.id) + '</td><td>' + esc(S.pedidos.labels[o.normalizedStatus] || o.orderStatus) + '</td><td class="nowrap">' + brl(c.receitaC != null ? c.receitaC / 100 : (o.totalAmount || 0)) + '</td><td class="nowrap">' + (c.resultadoC != null ? brlC(c.resultadoC) : '—') + '</td></tr>'; }).join('');
      var bloco7 = '<details class="panel"><summary style="cursor:pointer;padding:12px 16px;font-weight:700">7. Pedidos do Dia — ' + nn(vendas.n) + ' pedido(s) · ' + brl(vendas.valor) + '</summary><div class="pb"><div class="table-wrap"><table class="report"><thead><tr><th>Pedido</th><th>Status</th><th>Valor</th><th>Lucro</th></tr></thead><tbody>' + (pedRows7 || '<tr><td colspan="4" class="empty">Nenhum pedido pago neste dia.</td></tr>') + '</tbody></table></div></div></details>';
      // 8. EXPEDIÇÃO [recolhível] — histórico do que foi efetivamente bipado/confirmado neste dia
      // (nunca a fila operacional atual, que continua sendo só A ENVIAR — regra já aprovada).
      var expRows8 = exp.itens.map(function (i) { return '<tr class="rowlink" data-goped360="' + esc(i.orderId) + '"><td class="mono">' + esc(i.orderId) + '</td><td>' + esc(i.modalidade || '—') + '</td><td class="nowrap">' + (i.expedidoAt ? new Date(i.expedidoAt).toLocaleString('pt-BR') : '—') + '</td><td>' + esc(i.situacao) + '</td><td>' + (i.noAcelera ? '🟢 sim' : '🔴 não') + '</td></tr>'; }).join('');
      var bloco8 = '<details class="panel"><summary style="cursor:pointer;padding:12px 16px;font-weight:700">8. Expedição — ' + nn(exp.expedidos) + ' pedido(s) bipado(s)/confirmado(s)</summary><div class="pb">' +
        (exp.temSessao ? kv('Esperados / confirmados / faltaram', nn(exp.esperados) + ' / ' + nn(exp.expedidos) + ' / ' + nn(exp.faltaram)) : '<div class="footnote" style="margin-bottom:6px">Sem Sessão de Expedição registrada para este dia — mostrando apenas confirmações encontradas.</div>') +
        '<div class="table-wrap"><table class="report"><thead><tr><th>Pedido</th><th>Modalidade</th><th>Confirmado em</th><th>Situação</th><th>No Acelera</th></tr></thead><tbody>' + (expRows8 || '<tr><td colspan="5" class="empty">Nenhum pedido expedido/confirmado neste dia.</td></tr>') + '</tbody></table></div></div></details>';
      // 9. PENDÊNCIAS [recolhível] — as duas famílias (fluxo/financeiras + carteira) num único
      // checklist final, igual já aprovado, só recolhido por padrão.
      var pendFinRows9 = pend.financeiras.slice(0, 60).map(function (f) { return '<tr><td class="mono">' + esc(f.orderId) + '</td><td class="cell-text">' + esc(f.motivo) + '</td><td><button class="btn-sm" data-goped360="' + esc(f.orderId) + '">Ver pedido</button> <button class="btn-sm" data-caixapendfin="' + esc(f.orderId) + '">Resolver</button></td></tr>'; }).join('');
      var bloco9 = '<details class="panel" open><summary style="cursor:pointer;padding:12px 16px;font-weight:700">9. Pendências</summary><div class="pb">' +
        ...
        '<div class="kstrip" style="flex-wrap:wrap;margin-top:8px">' +
        ...
        '<div class="footnote" style="margin:10px 0 4px">Pendências financeiras dos pedidos (' + nn(pend.financeiras.length) + ')</div><div class="table-wrap"><table class="report"><thead><tr><th>Pedido</th><th>Problema</th><th></th></tr></thead><tbody>' + (pendFinRows9 || '<tr><td colspan="3" class="empty">Nenhuma pendência financeira neste dia. 🎉</td></tr>') + '</tbody></table></div>' +
        '</div></details>';
      var podeFechar = pend.total === 0 && pendCart.total === 0;
      // 10. FECHAR CAIXA
      var footer = '<div class="panel"><div class="ph"><h3>10. Fechar Caixa</h3></div><div class="pb" style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">' +
    ...
    function wire() {
      ...
      // §W-X do complemento: "+ Cadastrar nova conta" no próprio select — abre o modal, salva e
      // seleciona automaticamente a conta recém-criada, sem sair do fechamento.
      var contaSel = panel.querySelector('#bt-conta'); if (contaSel) contaSel.onchange = function () {
        if (contaSel.value !== '__novo__') return;
        contaSel.value = '';
        openCadastrarContaModal(function (full) { refresh(); setTimeout(function () { var s = panel.querySelector('#bt-conta'); if (s) s.value = full.id; }, 0); });
      };
      var btReg = panel.querySelector('#bt-registrar'); if (btReg) btReg.onclick = function () {
        ...
        var contaId = panel.querySelector('#bt-conta').value || null; if (contaId === '__novo__') contaId = null; var conta = contaId ? bankAccountLabel(contaId) : null; var obs = panel.querySelector('#bt-obs').value.trim() || null;
