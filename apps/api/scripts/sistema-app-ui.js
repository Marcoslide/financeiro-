...
(function () {
  ...
  function renderPosVenda() {
    ...
    app.querySelectorAll('[data-go]').forEach(function (b) { b.onclick = function () { if (b.dataset.arf) arF = b.dataset.arf; if (b.dataset.asub) analiseSub = b.dataset.asub; sub.posvenda = b.dataset.go; render(); }; });
  ...
  function devAnalises() {
    var subs = [['motivos', 'Motivos'], ['causas', 'Causas'], ['produtos', 'Produtos & SKUs'], ['achados', 'Achados']];
    var inner = analiseSub === 'causas' ? devCausas() : analiseSub === 'produtos' ? devCriticos() : analiseSub === 'achados' ? devAchados() : devMotivos();
  ...
  function bindAnalises() { app.querySelectorAll('[data-asub2]').forEach(function (b) { b.onclick = function () { analiseSub = b.dataset.asub2; render(); }; }); if (analiseSub === 'achados') bindAchados(); }
  ...
  function devExec() {
    ...
    var todo = [];
    if (conferir) todo.push(['Caixas chegaram — falta conferir', conferir, 'areceber', 'conferir']);
    if (aguard) todo.push(['Devoluções aguardando retorno', aguard, 'areceber', 'todos']);
    if (disp.vencendo || disp.vencidas) todo.push(['Disputas vencendo/vencidas', disp.vencendo + disp.vencidas, 'disputas', '']);
    if (semCausa) todo.push(['Devoluções sem causa interna', semCausa, 'ocorrencias', '']);
    ...
    return '<div class="panel"><div class="ph"><h3>Como estamos?</h3></div><div class="pb"><div class="cards6">' +
      ...
      (todo.length ? todo.map(function (x) { return '<div class="fin-line"><span>' + esc(x[0]) + '</span><span style="display:flex;gap:10px;align-items:center"><b>' + nn(x[1]) + '</b><button class="btn-sm primary" data-go="' + x[2] + '"' + (x[3] ? ' data-arf="' + x[3] + '"' : '') + '>Abrir</button></span></div>'; }).join('') : '<div class="footnote">Nada urgente no momento. 🎉</div>') +
      '</div></div>' +
      ...
      (motivos.length ? motivos.map(function (m) { return '<tr><td>' + esc((m.reason || '—').slice(0, 30)) + '</td><td>' + nn(m.cases) + '</td><td>' + brl(m.loss) + '</td></tr>'; }).join('') : '<tr><td colspan="3" class="empty">—</td></tr>') + '</tbody></table></div></div>' +
      '</div>' +

      '<div class="panel"><div class="ph"><h3>Quais produtos estão dando problema?</h3><button class="link-btn" data-go="analises" data-asub="produtos">Ver produtos</button></div><div class="table-wrap"><table><thead><tr><th>SKU</th><th>Produto</th><th>Ocor.</th><th>Perda</th><th>Causa</th></tr></thead><tbody>' +
      (crit.length ? crit.map(function (s) { return '<tr><td class="mono">' + esc(s.sku) + '</td><td>' + esc((s.product || '—').slice(0, 36)) + '</td><td>' + s.occ + '</td><td><b>' + brl(s.loss) + '</b></td><td>' + esc(s.dominant) + '</td></tr>'; }).join('') : '<tr><td colspan="5" class="empty">—</td></tr>') + '</tbody></table></div></div>';
  }
  ...
  function devDisputas() {
    ...
    var open = list.filter(function (o) { return ['POSSIVEL', 'EM_PREPARACAO', 'RESPONDIDA', 'AGUARDANDO_SHOPEE'].indexOf(o.disputeStatus) >= 0; });
    var venceHoje = open.filter(function (o) { return o.disputeDeadline && new Date(o.disputeDeadline).toDateString() === now.toDateString(); });
    var vence3 = open.filter(function (o) { return o.disputeDeadline && new Date(o.disputeDeadline) > now && new Date(o.disputeDeadline) <= soon; });
    var semResp = list.filter(function (o) { return o.disputeStatus === 'POSSIVEL'; });
    var linha = function (o) { return '<tr><td class="mono">' + esc(o.orderId || o.id) + '</td><td>' + esc((o.reason || '—').slice(0, 26)) + '</td><td>' + (DEV.DISPUTE_STATUS[o.disputeStatus] || o.disputeStatus) + '</td><td>' + (o.disputeDeadline ? dbr(o.disputeDeadline) : '—') + '</td><td>' + brl(o.requested) + '</td><td><button class="btn-sm" data-oc="' + esc(o.id) + '">Trabalhar</button></td></tr>'; };
    var tabela = function (title, arr, cls) { return '<div class="panel"><div class="ph"><h3>' + title + '</h3><span class="tag ' + (cls || '') + '">' + arr.length + '</span></div>' + (arr.length ? '<div class="table-wrap"><table><thead><tr><th>Pedido</th><th>Motivo</th><th>Status</th><th>Prazo</th><th>Valor</th><th></th></tr></thead><tbody>' + arr.map(linha).join('') + '</tbody></table></div>' : '<div class="pb"><div class="footnote">Nada aqui.</div></div>') + '</div>'; };
    return '<div class="cards6">' + fcard('Abertas', nn(d.abertas), '') + fcard('Vencendo (≤3d)', nn(d.vencendo), 'amber') + fcard('Vencidas', nn(d.vencidas), 'red') + fcard('Taxa de resposta', pct(d.taxaResposta), '') + fcard('Ganhas', nn(d.ganhas), 'green') + fcard('Perdidas', nn(d.perdidas), 'red') + fcard('Valor contestado', brl(d.contestado), '') + fcard('Valor recuperado', brl(d.recuperado), 'green') + '</div>' +
      tabela('Vence hoje — o que preciso defender', venceHoje, 'warn') + tabela('Vence em até 3 dias', vence3, 'warn') + tabela('Possível contestação (sem resposta)', semResp, 'info') +
      '<div class="footnote">Abra a ficha (Trabalhar) para registrar a resposta e o resultado da disputa.</div>';
  }
