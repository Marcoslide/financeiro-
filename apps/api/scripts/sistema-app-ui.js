...
(function () {
  ...
  function importPosVenda(type, file) {
    return file.arrayBuffer().then(function (ab) {
      ...
      Object.keys(groups).forEach(function (key) {
        ...
        next.orderId = rep.orderId; next.returnId = rep.returnId; next.status = rep.status; next.reason = rep.reason; next.resolution = rep.resolution || next.resolution;
        next.reasonRevised = rep.reasonRevised || next.reasonRevised; next.returnType = rep.returnType || next.returnType; next.sellerNote = rep.sellerNote || next.sellerNote; next.trackingStatus = rep.trackingStatus || next.trackingStatus;
        ...
        // Oportunidade de recurso vinda do próprio relatório (§27-28): "Ação do Vendedor solicitada até".
        // Classifica automaticamente como "Para recorrer" sem depender do operador criar a disputa.
        if (rep.sellerActionDeadline) { var dl = new Date(rep.sellerActionDeadline); if (!isNaN(dl)) { next.disputeDeadline = dl.toISOString(); next.hasSellerWindow = true; if (next.disputeStatus === 'NAO_INICIADA') next.disputeStatus = 'POSSIVEL'; } }
        next.items = g.map(function (r) { return { sku: r.sku, productName: r.productName, variationName: r.variationName, qty: r.quantity, unitPrice: r.unitPrice, skuLinked: !!(r.sku && skuCost[r.sku.toLowerCase()]) }; });
  ...
  function renderPosVenda() {
    var tabs = [['visao', 'Visão Geral'], ['casos', 'Casos'], ['recebimentos', 'Recebimentos'], ['disputas', 'Disputas'], ['analises', 'Análises'], ['planos', 'Plano de Ação'], ['import', 'Importações']];
    // compat com chaves antigas
    if (sub.posvenda === 'ocorrencias') sub.posvenda = 'casos';
    ...
    app.querySelectorAll('[data-go]').forEach(function (b) { b.onclick = function () {
      ...
      if (dest === 'casos') { devF.internalStatus = ''; devF.disputeStatus = ''; devF.search = ''; devF.type = b.dataset.oct || ''; devF.flag = b.dataset.ocf || ''; devPage = 1; }
  ...
  function planResult(p) {
    var higher = !!INDICATOR_HIGHER[p.indicatorKind]; var b = p.baselineValue, c = measureIndicator(p);
  ...
  function planCard(p) {
    ...
    return '<div class="panel"><div class="ph"><h3>' + esc(p.title) + '</h3><span class="tag ' + (p.status === 'ENCERRADO' ? 'ok' : p.status === 'MEDINDO' ? 'info' : 'neutral') + '">' + (PLAN_STATUS[p.status] || p.status) + '</span></div><div class="pb">' +
      ...
      '<div class="fin-line"><span>Como vamos medir?</span><span>' + esc(p.indicator) + '</span></div>' +
      '<div class="kstrip" style="margin:10px 0"><div class="kc"><div class="kl">Antes</div><div class="kv" style="font-size:18px">' + fmtInd(p, b) + '</div></div><div class="kc"><div class="kl">Agora</div><div class="kv" style="font-size:18px">' + fmtInd(p, c) + '</div></div><div class="kc"><div class="kl">Meta</div><div class="kv" style="font-size:18px">' + (p.targetValue != null ? fmtInd(p, p.targetValue) : '—') + '</div></div><div class="kc ' + res.cls + '"><div class="kl">Resultado</div><div class="kv" style="font-size:16px">' + res.label + '</div></div></div>' +
      (p.implementedAt ? '<div class="fin-line"><span>Implantado em</span><span>' + dbr(p.implementedAt) + '</span></div><div class="footnote" style="margin:2px 0 0">Maturidade da medição</div><div style="height:10px;border-radius:6px;background:var(--line);overflow:hidden;margin-top:3px"><div style="height:100%;width:' + mat + '%;background:var(--info)"></div></div><div class="footnote" style="margin:2px 0 8px">' + mat + '% da janela de ' + (p.windowAfterDays || 30) + ' dias</div>' : '') +
  ...
  function devOcc() {
    ...
    return secHead('CASOS', 'Casos', 'Todos os casos de devolução, cancelamento e falha de entrega em um só lugar. Localize, filtre, abra e trabalhe o caso.') +
      ...
      activeFilter + '<div class="count-line"><b>' + nn(list.length) + '</b> casos</div>' +
      ...
      slice.map(function (o) { var it = (o.items || [])[0] || {}; var sit = situacaoCaso(o); var prod = (it.productName || '—') + (it.variationName ? ' · ' + it.variationName : '') + ((o.items || []).length > 1 ? ' (+' + (o.items.length - 1) + ')' : ''); return '<tr><td class="mono">' + esc(o.orderId || '—') + '</td><td>' + esc(TYPE_LABELS[o.type] || '—') + '</td><td class="cell-text">' + esc(prod) + '<div class="footnote" style="margin:0">' + esc(it.sku || '—') + '</div></td><td class="cell-text">' + esc(o.reason || '—') + '</td><td><span class="pill st-int">' + (DEV.INTERNAL_STATUS[o.internalStatus] || o.internalStatus) + '</span></td><td>' + esc(o.ownerName || '—') + '</td><td>' + (o.disputeStatus !== 'NAO_INICIADA' ? '<span class="tag info">' + DEV.DISPUTE_STATUS[o.disputeStatus] + '</span>' : '<span class="footnote">—</span>') + '</td><td class="nowrap">' + (o.impact && o.impact.knownNetImpact != null ? '<b>' + brl(o.impact.knownNetImpact) + '</b>' : '—') + '</td><td><span class="tag ' + sit[1] + '">' + sit[0] + '</span></td><td><button class="btn-sm" data-oc="' + esc(o.id) + '">Abrir</button></td></tr>'; }).join('') +
